...
-- =============================================================================
-- `D-37` — مهلةُ استرجاعِ الحجزِ المتروكِ هيَ مهلةُ **مدينةِ الصفِّ**.
--
-- **السابقةُ:** `20260922050000_sec_19_claim_undeliverable_guard.sql` — آخرُ
-- تعريفٍ لـ`claim_notification_delivery()`. والنافذُ هوَ الأخيرُ.
--
-- **المسألةُ:** كانت الدالّةُ تقرأُ `notification_claim_timeout_seconds` بـ
-- `where key = … limit 1` بلا مدينةٍ ولا ترتيبٍ، ثمَّ تسترجعُ صفوفَ **كلِّ** المدنِ
-- بتلكَ القيمةِ. والإعدادُ لكلِّ مدينةٍ (`unique (city_id, key)`)، فمدينتانِ
-- بمهلتينِ مختلفتينِ تُعطيانِ مهلةً غيرَ حتميّةٍ: صفٌّ يُسلَبُ من عاملٍ حيٍّ قبلَ
-- أوانِه (فتتكرّرُ رسالتُه)، أو يتأخّرُ استرجاعُه صامتاً.
--
-- **العلاجُ:** شرطُ الاسترجاعِ يقرأُ مهلةَ `n.city_id` لكلِّ صفٍّ، والافتراضُ
-- 300 والحدُّ الأدنى 1 كما كانا. وبقيّةُ الدالّةِ منسوخةٌ حرفاً عن السابقةِ.
-- وهذا ما تفعلُه أختُها `claim_subscription_notices(p_city_id)` أصلاً.
--
-- **ولا يُدَّعى:** لا تغييرَ في قيمِ الإعدادِ، ولا في اختيارِ الصفِّ المُسلَّمِ،
-- ولا في فروعِ حلِّ العنوانِ وحرسِ `SEC-19`. (`ADR 0196`)
-- =============================================================================

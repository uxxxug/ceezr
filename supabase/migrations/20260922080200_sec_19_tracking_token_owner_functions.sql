...
-- =============================================================================
-- SEC-19 — بندُ الترتيبِ ٣: ربطُ دوالِّ رموزِ التتبُّعِ و`erase_my_account`
--   و`export_my_data` بـ`created_by_user_id` (الطورُ الثاني: تحديثُ الدوالّ)
--
--   تحديثُ أربعِ دوالّ لتستعمِلَ `created_by_user_id` (uuid، ثابتٌ) بدلاً من
--   `created_by` (bigint، معرّفُ تيليجرام) — معَ احتفاظٍ بالعمودِ القديمِ
--   مسلكاً تراثيّاً للصفوفِ التي لم تُعَيَّن.
--
--   **ما لا تُغيِّرُه:** التواقيعُ العامةُ لا تُمسُّ — `p_telegram_id` يبقى
--   الوسيطَ، ويُحَلُّ `users.id` داخليًّا.
-- =============================================================================

-- ────────────────────────────────────────────────────────────────────────────
-- ١) `issue_tracking_token`: كتابةُ العمودينِ معًا
-- ────────────────────────────────────────────────────────────────────────────
...
begin
  ...
  -- SEC-19 بندُ ٣: نلتقطُ `users.id` أيضًا لنكتبَه في `created_by_user_id`.
  select u.telegram_id, u.id
    into v_owner_tg, v_owner_id
    from riders r
  ...
  insert into trip_tracking_tokens (city_id, order_id, token, created_by, created_by_user_id, expires_at)
  values (v_order.city_id, v_order.id, p_token, p_telegram_id, v_owner_id, v_expires_at);
...
comment on function issue_tracking_token(uuid, bigint, text) is
  'يُصدر رمزَ تتبّعٍ لطلبٍ قائم بعد التحقّق من ملكيته ذرّياً — المهلة من platform_settings (§4.2). SEC-19 بندُ ٣: يكتبُ `created_by_user_id` (uuid) معَ `created_by` (bigint) التراثيِّ.';

-- ────────────────────────────────────────────────────────────────────────────
-- ٢) `revoke_tracking_token`: المطابقةُ بالمالكِ الثابتِ معَ احتفاظٍ تراثيٍّ
-- ────────────────────────────────────────────────────────────────────────────
...
declare
  ...
  v_actor_id uuid;
begin
  -- SEC-19 بندُ ٣: حَلُّ `users.id` من `p_telegram_id` للمطابقةِ بالعمودِ الجديد.
  -- وإن لم يُوجَدْ مستخدمٌ (مُعرّفٌ تيليجرام غيرُ مسجَّلٍ) فلا رمزَ له أصلاً.
  select u.id into v_actor_id from users u where u.telegram_id = p_telegram_id limit 1;
  ...
  update trip_tracking_tokens
   ...
   where token = p_token
     ...
     and (
       (v_actor_id is not null and created_by_user_id = v_actor_id)
...
comment on function revoke_tracking_token(text, bigint) is
  'يُلغي رمزَ تتبّعٍ بطلب مُصدِره ذرّياً — سببٌ واحد لكلّ فشلٍ فلا يصير أداةَ استكشاف (§4.2). SEC-19 بندُ ٣: يطابقُ بـ`created_by_user_id` معَ احتفاظٍ تراثيٍّ.';

-- ────────────────────────────────────────────────────────────────────────────
-- ٣) `revoke_order_tracking_tokens`: المطابقةُ بالمالكِ الثابتِ معَ احتفاظٍ
-- ────────────────────────────────────────────────────────────────────────────
...
begin
  ...
  -- SEC-19 بندُ ٣: حَلُّ `users.id` من `p_telegram_id`.
  select u.id into v_actor_id from users u where u.telegram_id = p_telegram_id limit 1;
  ...
  update trip_tracking_tokens tt
   ...
   where tt.order_id = p_order_id
     ...
     and (
       (v_actor_id is not null and tt.created_by_user_id = v_actor_id)
       or (tt.created_by_user_id is null and exists (
         select 1
           from orders o
...
comment on function revoke_order_tracking_tokens(uuid, bigint) is
  'إلغاءُ كلّ روابط تتبّع طلبٍ بطلب صاحبه — بمعرّف الطلب لا بالرمز (§4.2). SEC-19 بندُ ٣: يطابقُ بـ`created_by_user_id` معَ احتفاظٍ تراثيٍّ.';

-- ────────────────────────────────────────────────────────────────────────────
-- ٤) `export_my_data`: تصديرُ رموزِ التتبُّعِ بالمالكِ الثابتِ معَ احتفاظٍ
-- ────────────────────────────────────────────────────────────────────────────
...
comment on function export_my_data(bigint) is
  'تُصدِّرُ بياناتِ صاحبِ الحسابِ لتحميلها قبلَ التجهيلِ (F2-11 / ADR 0112). SEC-19 بندُ ٣: تصديرُ رموزِ التتبُّعِ بـ`created_by_user_id` الثابتِ لا بـ`telegram_id`.';

-- ────────────────────────────────────────────────────────────────────────────
-- ٥) `erase_my_account`: الحذفُ بالمالكِ الثابتِ معَ احتفاظٍ تراثيٍّ
-- ────────────────────────────────────────────────────────────────────────────
...
comment on function erase_my_account(bigint) is
  'تُجهّلُ حسابَ صاحبه وتُعيدُ إيصالاً مُفصَّلاً (F2-11 / ADR 0112). SEC-19 بندُ ٣: حذفُ رموزِ التتبُّعِ بـ`created_by_user_id` الثابتِ لا بـ`telegram_id`.';

-- لا تُعادُ المنحُ: `create or replace` يحفظُها كما كانت (service_role وحدَها).

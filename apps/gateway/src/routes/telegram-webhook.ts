...
export function createTelegramWebhookRoutes(deps: WebhookDependencies): Hono {
  ...
  app.post("/webhook/telegram/:bot", async (c) => {
    ...
    // ٤) رفضُ الحمولةِ بلا `update_id` صالحٍ قبلَ أيِّ إيداعٍ أو معالجةٍ.
    //    [Telegram Bot API](https://core.telegram.org/bots/api): `update_id` هو الحقلُ
    //    الإلزاميُّ الوحيدُ في كائنِ `Update` — فحمولةٌ بلا رقمٍ صحيحٍ موجبٍ ليست
    //    تحديثَ تيليجرام، ولا مفتاحَ لها يُمنَعُ به التكرارُ، فلا تُدرَجُ في الطابور.
    //    ومعالجتُها داخلَ طلبِ `HTTP` تنقضُ ADR 0054 §٦ («العملُ الذي يلي الإيصالَ...
    //    لا يُنفَّذ داخلَ طلبِ HTTP»). فيُرفضُ هذا الصنفُ بـ400 قبلَ الإيداعِ، في
    //    مسارَي الإنتاجِ والتدهورِ معاً.
    if (updateId === null) {
      deps.log?.("رُفض تحديث بلا update_id صالح", { bot });
      return c.json({ ok: false, error: "INVALID_UPDATE" }, 400);
    }

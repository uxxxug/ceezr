 ...
 *
 * الحالة: **مكتوب وغير مركَّب** — لا يُستدعى `createTrackingRoutes` من
 *   `apps/gateway/src/index.ts` ولا من `container.ts`. مغطّى باختبارات وحدة
 *   (`tests/unit/tracking-route.test.ts`) تُشغّل Hono مستقلّاً، وهذا كل تشغيله.
 * ينتمي إلى: apps/gateway/src/routes
 * يُتوقع أن يستخدمه لاحقاً: الواجهة الأمامية، بوت السائق
 *
 * ## لماذا غير مركَّب — ولا يُركَّب بضغطة
 *
 * المسار الحيّ للتتبّع في الإنتاج ليس هذا: موقعُ السائق يأتي رسالةَ موقعٍ من
 * تلغرام إلى `handleLocation`، فيُقيَّم بـ`assessGpsFix` ويُكتب بـ
 * `drivers.updateLocation` (الكاتب الوحيد، ADR-0015)، ثم تُركَّب الجلسة والنشر في
 * `packages/application/tracking/live-tracking.ts` الموصول في
 * `apps/gateway/src/container.ts`.
 *
 * وتركيب هذا الملف فوق ذلك يفتح ثلاثة أبواب لا يُغلقها التركيب نفسه:
 *   ١. `TrackingTokenStore` واجهةٌ بلا أيّ تنفيذ في المستودع، ولا مُصدِرَ رموزٍ
 *      يمنح السائق رمزاً. فتركيب المسار يعني نظام مصادقةٍ ثانياً من الصفر.
 *   ٢. تقييمُ الإصلاحة يجري مرّتين — مرّةً في الحوار ومرّةً في `TrackingService`.
 *   ٣. يصير لـ«الإصلاحة السابقة» مصدران، فتتناقض الخريطة مع القاعدة.
 *
 * فلا يُحذف (فيه عملٌ صحيحٌ لمرحلة تتبّع العميل: مصادقةٌ تشتقّ `driverId` من
 * الرمز لا من الـbody)، ولا يُفعَّل بلا قرار مالكٍ يشمل مُصدِر الرموز ودمج
 * التقييم في مسارٍ واحد.
 */

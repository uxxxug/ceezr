...
describe("السجلُّ الحقيقيُّ — أرقامٌ مقيسةٌ مُثبَّتةٌ", () => {
   ...
   */
  it("ثمانيةٌ وخمسون ملفّاً و544 حالةً — 514 قياسُ OPS-009 و12 لـOPS-006 و8 لـBUG-009 و10 لـBUG-001", () => {
    expect(SKIP_REGISTRY).toHaveLength(58);
    expect(SKIP_REGISTRY.reduce((sum, entry) => sum + entry.skipped, 0)).toBe(544);
  });

...
describe("السجلُّ الحقيقيُّ — أرقامٌ مقيسةٌ مُثبَّتةٌ", () => {
   ...
   * والسبعُ الزائدةُ في الملفِّ السابعِ والخمسين قياسُ `BUG-009` يومَ 2026-09-02 —
   ...
   */
  it("سبعةٌ وخمسون ملفّاً و533 حالةً — 514 قياسُ OPS-009 و12 لـOPS-006 و7 لـBUG-009", () => {
    expect(SKIP_REGISTRY).toHaveLength(57);
    expect(SKIP_REGISTRY.reduce((sum, entry) => sum + entry.skipped, 0)).toBe(533);
  });

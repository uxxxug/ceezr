...
describe("السجلُّ الحقيقيُّ — أرقامٌ مقيسةٌ مُثبَّتةٌ", () => {
   ...
   * الهجرةِ الآمنةِ، والثلاثَ عشرةَ قياسُ `F6-06`، والستُّ قياسُ `F4-01`؛ ولا يُقاسُ
   * شيءٌ من ذلك إلّا على محرِّكٍ حقيقيٍّ فيُتجاوَزُ حيث لا `TEST_DATABASE_URL`.
   * **والأعدادُ تُرفَع ولا تُخفَّض**: الاختبارُ حرزٌ على التصنيفِ لا سقفٌ على العملِ.
   */
  it("خمسةٌ وسبعونَ ملفّاً و657 حالةً بعد F7-07 وF6-06 وF4-01", () => {
    expect(SKIP_REGISTRY).toHaveLength(75);
    expect(SKIP_REGISTRY.reduce((sum, entry) => sum + entry.skipped, 0)).toBe(657);
  });

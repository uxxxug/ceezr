...
describe("السجلُّ الحقيقيُّ — أرقامٌ مقيسةٌ مُثبَّتةٌ", () => {
   ...
   */
  it("تسعةٌ وخمسون ملفّاً و551 حالةً — 514 قياسُ OPS-009 و12 لـOPS-006 و8 لـBUG-009 و10 لـBUG-001 و7 لـBUG-005", () => {
    expect(SKIP_REGISTRY).toHaveLength(59);
    expect(SKIP_REGISTRY.reduce((sum, entry) => sum + entry.skipped, 0)).toBe(551);
  });

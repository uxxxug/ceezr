...
describe("السجلُّ الحقيقيُّ — أرقامٌ مقيسةٌ مُثبَّتةٌ", () => {
   ...
   */
  it("ستّةٌ وخمسون ملفّاً و526 حالةً — 514 قياسُ OPS-009 و12 لملفِّ OPS-006", () => {
    expect(SKIP_REGISTRY).toHaveLength(56);
    expect(SKIP_REGISTRY.reduce((sum, entry) => sum + entry.skipped, 0)).toBe(526);
  });

...
describe("السجلُّ الحقيقيُّ — أرقامٌ مقيسةٌ مُثبَّتةٌ", () => {
   ...
   * `TEST_DATABASE_URL`.
   * والسبعُ الزائدةُ في الملفِّ الثالثِ والسبعينَ قياسُ `F7-07` / `CAP-007` — أدواتُ
   * الهجرةِ الآمنةِ: تطبيقُ السلسلةِ، وإعادةُ تطبيقِها بلا تضاعُفٍ، وارتدادُ ملفٍّ
   * ساقطٍ كاملاً، وطورُ الفهرسِ بلا معاملةٍ، وسقوطُها بـ`55P03` عندَ قفلٍ حاجزٍ؛
   * ولا يُقاسُ شيءٌ من ذلك إلّا على محرِّكٍ حقيقيٍّ فيُتجاوَزُ حيث لا
   * `TEST_DATABASE_URL`.
   * **والأعدادُ تُرفَع ولا تُخفَّض**: الاختبارُ حرزٌ على التصنيفِ لا سقفٌ على العملِ.
   */
  it("ثلاثةٌ وسبعونَ ملفّاً و638 حالةً — 514 قياسُ OPS-009 و12 لـOPS-006 و8 لـBUG-009 و10 لـBUG-001 و7 لـBUG-005 و7 لـBUG-008 و5 لـBUG-002 و3 لـBUG-003 و34 لـBUG-004 و7 لـCAP-003 و6 لـCAP-002 و5 لـF5-04 و5 لـF5-08 و8 لـF6-05", () => {
    expect(SKIP_REGISTRY).toHaveLength(73);
    expect(SKIP_REGISTRY.reduce((sum, entry) => sum + entry.skipped, 0)).toBe(638);
  });

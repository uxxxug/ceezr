...
describe("السجلُّ الحقيقيُّ — أرقامٌ مقيسةٌ مُثبَّتةٌ", () => {
   ...
   * `TEST_DATABASE_URL`.
   * والتسعُ الزائدةُ في الملفِّ الثالثِ والسبعينَ قياسُ `F7-02` / `CAP-005` — خطّةُ
   * التنفيذِ قبلَ الفهرسِ وبعدَه: لكلِّ فهرسٍ من الستّةِ يلزمُ ظهورُه في الخطّةِ حينَ
   * يكونُ قائماً، وانحدارُ الخطّةِ إلى مسحٍ كاملٍ أو فرزٍ وارتفاعُ كلفتِها حينَ
   * يُسقَطُ في معاملةٍ تُرتَدُّ؛ ولا يُقاسُ حكمُ المخطِّطِ إلّا على محرِّكٍ حقيقيٍّ
   * فيُتجاوَزُ حيث لا `TEST_DATABASE_URL`.
   * **والأعدادُ تُرفَع ولا تُخفَّض**: الاختبارُ حرزٌ على التصنيفِ لا سقفٌ على العملِ.
   */
  it("ثلاثةٌ وسبعونَ ملفّاً و640 حالةً — 514 قياسُ OPS-009 و12 لـOPS-006 و8 لـBUG-009 و10 لـBUG-001 و7 لـBUG-005 و7 لـBUG-008 و5 لـBUG-002 و3 لـBUG-003 و34 لـBUG-004 و7 لـCAP-003 و6 لـCAP-002 و5 لـF5-04 و5 لـF5-08 و8 لـF6-05 و9 لـF7-02", () => {
    expect(SKIP_REGISTRY).toHaveLength(73);
    expect(SKIP_REGISTRY.reduce((sum, entry) => sum + entry.skipped, 0)).toBe(640);
  });

...
describe("السجلُّ الحقيقيُّ — أرقامٌ مقيسةٌ مُثبَّتةٌ", () => {
   ...
   * `TEST_DATABASE_URL`.
   * والثلاثَ عشرةَ الزائدةُ في الملفِّ الثالثِ والسبعينَ قياسُ `F6-06` — الضغطُ
   * العكسيُّ لكلِّ طابورٍ: أنَّ حكمَ القاعدةِ وحكمَ الشيفرةِ يتّفقانِ على الحِمْلِ
   * نفسِه، وأنَّ نافذةَ الموتى لها معنىً، وأنَّ الحرجَ يمرُّ والبثَّ يُؤجَّلُ وحدةً
   * واحدةً، وأنَّ سقفَ تزامنِ المستهلِكِ ومنتِجِ طابورِ تيليجرام يردّانِ بسببٍ
   * مُعلَنٍ؛ والمقصودُ حكمُ المُطلِّبِ داخلَ المعاملةِ وحفظُه بـ`set_config` وقيدُ
   * أثرِ الدقيقةِ — لا يُقاسُ إلّا على قاعدةٍ حقيقيّةٍ فيُتجاوَزُ حيث لا
   * `TEST_DATABASE_URL`.
   * **والأعدادُ تُرفَع ولا تُخفَّض**: الاختبارُ حرزٌ على التصنيفِ لا سقفٌ على العملِ.
   */
  it("أربعةٌ وسبعونَ ملفّاً و650 حالةً — 514 قياسُ OPS-009 و12 لـOPS-006 و8 لـBUG-009 و10 لـBUG-001 و7 لـBUG-005 و7 لـBUG-008 و5 لـBUG-002 و3 لـBUG-003 و34 لـBUG-004 و7 لـCAP-003 و6 لـCAP-002 و5 لـF5-04 و5 لـF5-08 و8 لـF6-05 و6 لـF4-01 و13 لـF6-06", () => {
    expect(SKIP_REGISTRY).toHaveLength(74);
    expect(SKIP_REGISTRY.reduce((sum, entry) => sum + entry.skipped, 0)).toBe(650);
  });

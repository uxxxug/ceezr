 ...
 * (كانَ 20s؛ رُفِعَ معَ المهلةِ الأولى كي يبقى بعدَها ما يتّسعُ لقراءاتِ السكونِ.)
 */
const SETTLE_TIMEOUT_MS = 30_000;
const SETTLE_INTERVAL_MS = 500;
/**
 * انتظارٌ أوّليٌّ **أطولُ من `PGSTAT_IDLE_INTERVAL`** (عشرُ ثوانٍ في PostgreSQL ≥15).
 *
 * تصحيحٌ بالإضافةِ (`D-35` · `ADR 0191` · `ح-8`): كانَ ههنا 2.5s بحُجّةِ أنَّ المهلةَ
 * الدُّنيا `PGSTAT_MIN_INTERVAL` ثانيةٌ. **والحُجّةُ ناقصةٌ**: خادمٌ خلفيٌّ أفرغَ قبلَ
 * أقلَّ من ثانيةٍ ثمَّ خمَلَ لا يُفرِغُ بعدَ ثانيةٍ بل **بعدَ عشرٍ** (`pgstat_report_stat`
 * يعيدُ `PGSTAT_IDLE_INTERVAL` مهلةً للخمولِ)، فثلاثُ قراءاتٍ متطابقةٍ خلالَ ~4s سكونٌ
 * كاذبٌ. وقيسَ أثرُه: عملُ إحماءِ فهارسِ النظامِ (~9k صفٍّ) يُحتسَبُ في الكتلةِ الدافئةِ
 * عادةً (14525) ويسبقُها أحياناً (1189) على الالتزامِ عينِه — فسقطَ الحاجزُ ×4.52 في
 * `36122019657` ومرَّ ×0.37 في `36122025751`. وبعدَ الإصلاحِ محلّيّاً: ×1.11 · ×1.00.
 * ويحرسُ القيمةَ `settle.idle-flush` في `scripts/lib/soak-work-measure.ts`.
 */

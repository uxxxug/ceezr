...
## الخلاصة المثبتة بالأرقام

الـskeleton الموجود في HTML الأولي **يُرسم مبكرًا** (firstPaint عند 656–693ms على Slow 4G)، لكن **نصه الوحيد مخفي بصريًا** بـ`clip:rect(0 0 0 0)` و`width:1px;height:1px`. متصفح Chromium لا يحتسب النص المخفي كـ«contentful». النتيجة: FCP لا يُشعَّل إلا بعد أن يحمّل React ويُرسم نصًا مرئيًا — عند 2224ms (Slow 4G + CPU ×4).
...
- **يُنفَّذ بعد FCP** في كل السيناريوهات (الـmodule script مؤجَّل، يُنفَّذ بعد تحميل وتقييم الـimports).
...
- **في حالة النص المخفي:** الشبكة مهيمنة (94%) — FCP ينتظر تحميل JS bundles.
- **في حالة النص المرئي:** الشبكة مهيمنة (100%) — FCP يحدث بعد تحميل HTML فقط.
- **CPU ×4 تأثيره ضئيل** (~5%).
...
**قابلة للعلاج ضمن الكود.** الاختناق ليس من بيئة القياس — الشبكة وCPU يعملان كما هو متوقع. المشكلة أن نص الـskeleton مخفي بصريًا بطريقة تجعل Chrome لا يحتسبه كـ«contentful».
...
**الشبكة مهيمنة** (94% في حالة النص المخفي). **CPU ×4 يضيف ~5% فقط.** جعل نص الـskeleton مرئيًا **يحل المشكلة جذريًا** — FCP يقفز من 2224ms إلى 680ms.

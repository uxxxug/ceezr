...
export const ROLLBACK_DECLARATIONS: readonly RollbackDeclaration[] = [
  ...
  },
  {
    ...
    why: "الهجرةُ `20260920180000` (`PD-020` · `ADR 0159`) تُضيفُ إلى حمولةِ المطالبةِ حقلاً واحدًا: `reason` من صفِّ الحادثِ — فالبطاقةُ المُرسَلةُ لقروبِ الإسنادِ تُختارُ قالبَها بجنسِ البلاغِ (طوارئُ سلامةٍ أو قرارُ إسنادٍ) قبلَ فتحِ القروبِ. والمنطقُ نُسِخَ حرفًا بحرفٍ من `20260918010000` وما زادَ الحقلَ وحدَه، والسحبُ بعدَ `create or replace` إعادةُ إحكامِ السطحِ لا تضييقٌ. ولا مسارَ سابقًا يُخفِقُ: الحقولُ القديمةُ كلُّها بحروفِها، وعاملٌ قديمٌ يقرأُ الحمولةَ بلا `reason` فيبني بطاقةَ الاستغاثةِ كما كانَ — والصفُّ يُسلَّمُ ويُختَمُ لا يعلَقُ. والعودةُ بالصورةِ السابقةِ تكفي: تُقرأُ بطاقاتُ التعذُّرِ بقالبِ الاستغاثةِ الواحدِ — تراجعُ تمييزٍ لا فقدُ تسليمٍ.",
    breaksPreviousRelease: false,
    rollbackPath: "code-only",
    coupledDeploy: true,
    owner: "منفّذ المستودع",
  ...
  {
    migration: "20260918040000_f12_05_mutual_rating_driver_summary.sql",
    change: "revoke_function:completed_ride_summary(2)",
    why: "الهجرةُ تُعيدُ تعريفَ `completed_ride_summary` بـ`create or replace` **بالتوقيعِ والمردِّ نفسِها** (F12-05): تُوسِّعُ المنظورَ من راكبٍ وحده إلى راكبٍ **أو** سائقٍ مُسنَدٍ — والاتّجاهُ يُستنتَجُ من علاقةِ الطلبِ لا من دورٍ مُسبَقٍ. والسحبُ إعادةُ قفلِ السطحِ كما كانَ، و`create or replace` تُعيدُ المنحَ المبدئيَّ لـ`public` فلا يُترَكُ ذلكَ لحُسنِ الظنِّ. **ولا كسرَ لنسخةٍ سابقةٍ**: عقدُ الراكبِ القائمُ محفوظٌ حرفاً (`driver` و`rating` كما كانَا)، والزيادةُ `rider` و`direction` حقولٌ جديدةٌ لا تُقرأُها شيفرةُ الأمسِ. والعودةُ إعادةُ تطبيقِ النسخةِ السابقةِ من `20260914010000` وحدَها.",
    breaksPreviousRelease: false,
    rollbackPath: "code-only",
    coupledDeploy: false,
    owner: "منفّذ المستودع",
  ...
  {
    migration: "20260918030000_f12_07_lost_item_mechanism.sql",
    change: "revoke_function:notification_kind_priority(1)",
    why: "الهجرةُ `20260918030000` (`F12-07`) تُعيدُ تعريفَ `notification_kind_priority(text)` بـ`create or replace` **بالتوقيعِ والمردِّ نفسِهما** لتُضيفَ فرعاً واحداً يُرجِعُ رتبةَ `lost_item_report` (=3، متوسّطٌ) — وهوَ بلاغُ دعمٍ فيه قيمةٌ زمنيّةٌ لا استغاثةٌ ولا مؤقِّتُ إسنادٍ. وبقيةُ الفروعِ منسوخةٌ حرفاً بحرفٍ ولم يُمَسَّ مفتاحٌ ولا توقيعٌ. والسحبُ بعدَه **إعادةُ قفلِ السطحِ كما كانَ لا تضييقٌ جديدٌ**: الدالّةُ كانت لـ`service_role` وحدَها قبلَ التغييرِ وتبقى كذلكَ بعدَه، و`create or replace` لا تُسقِطُ منحاً قائماً أصلاً — فالسطرُ تكرارٌ مقصودٌ ليكفيَ الملفُّ بذاتِه إن أُعيدَ بناءُ القاعدةِ. **ولا كسرَ لنسخةٍ سابقةٍ**: المردُّ لم يزدَ مفتاحاً ولم يفقدْه، فشيفرةٌ لا تعرفُ `lost_item_report` تُرجِعُ `3` لكلِّ نوعٍ غيرِ مذكورٍ كما فعلَت بالأمسِ. **والعودةُ بالصورةِ السابقةِ تكفي**: أقصى أثرِها أنَّ `lost_item_report` يعودُ يُحكَمُ بالأقدميّةِ لا بالرتبةِ — وهوَ عيبُ ترتيبٍ يُخفَقُ في مدينةٍ مُشبَعةٍ لا فقدُ بياناتٍ. والنشرُ **مقرونٌ** لأنَّ معالجَ التسليمِ ونصوصَ الإخبارِ تُشحَنُ مع المخطّطِ.",
    breaksPreviousRelease: false,
    rollbackPath: "code-only",
    coupledDeploy: true,
    owner: "منفّذ المستودع",
  ...
  {
    migration: "20260918030000_f12_07_lost_item_mechanism.sql",
    change: "revoke_function:resolve_notification_recipient(1)",
    why: "الهجرةُ `20260918030000` (`F12-07`) تُعيدُ تعريفَ `resolve_notification_recipient(text)` بـ`create or replace` **بالتوقيعِ والمردِّ نفسِهما** لتُضيفَ فرعاً يحلُّ سائقَ الطلبِ المُسنَدَ مستقبِلاً لبلاغِ المفقودِ (`lost_item_report`): يقرأُ `orders.assigned_driver_id` من الطلبِ الذي حُمِلَ في `payload->>'order_id'` ويُرجِعُه `recipient_user_id`، ويرفَعُ رأسَ صفِّ `drivers` إلى `users` كما يفعلُ فرعُ الإلغاءِ. وبقيةُ الفروعِ منسوخةٌ حرفاً بحرفٍ. والسحبُ بعدَه **إعادةُ قفلِ السطحِ كما كانَ لا تضييقٌ جديدٌ**. **ولا كسرَ لنسخةٍ سابقةٍ**: الفروعُ القديمةُ بأحكامِها، ونوعٌ جديدٌ لم يكنْ يُحَلُّ من قبلُ يُرجِعُ `null` للصنفِ العامِّ القديمِ كما كانَ. **والعودةُ بالصورةِ السابقةِ تكفي**: أقصى أثرِها أنَّ صفوفَ `lost_item_report` تُصنَّفُ بلا مُستقبِلٍ فيُعدُّها القارئُ منزَّهةً — وذاكَ عيبُ إخبارٍ لا فقدُ بياناتٍ. والنشرُ **مقرونٌ**.",
    breaksPreviousRelease: false,
    rollbackPath: "code-only",
    coupledDeploy: true,
    owner: "منفّذ المستودع",
  ...
  },
  {
    migration: "20260918030000_f12_07_lost_item_mechanism.sql",
    change: "revoke_function:claim_notification_delivery(0)",
    why: "الهجرةُ `20260918030000` (`F12-07`) تُعيدُ تعريفَ `claim_notification_delivery()` بـ`create or replace` **بالتوقيعِ نفسِه ورموزِ الرفضِ نفسِها** لتُضيفَ `lost_item_report` إلى `v_ride_kinds` (فالمُصَرِّفُ الرئيسيُّ يلتقطُه كما يلتقطُ الإلغاءَ) وفرعاً للإغناءِ يقرأُ `telegram_id` و`language_code` للسائقِ من `drivers` و`users` حيّاً لحظةَ الالتقاطِ — كما يفعلُ فرعُ الإلغاءِ. وبقيةُ الفروعِ والبنيةِ منسوخةٌ حرفاً بحرفٍ. والسحبُ بعدَه **إعادةُ قفلِ السطحِ كما كانَ لا تضييقٌ جديدٌ**: الدالّةُ كانت لـ`service_role` وحدَها قبلَ التغييرِ وتبقى كذلكَ بعدَه، و`create or replace` لا تُسقِطُ منحاً قائماً أصلاً. **ولا كسرَ لنسخةٍ سابقةٍ**: المردُّ بمفاتيحِه نفسِها، والصفوفُ التي لا يعرفُها الفرعُ القديمُ تتجاوزُها فلا تُلتقَطُ — وهوَ سلوكُ الأمسِ. **والعودةُ بالصورةِ السابقةِ تكفي**: أقصى أثرِها أنَّ صفوفَ `lost_item_report` لا يلتقطُها أحدٌ فتبقى `pending` حتى يُعادَ نشرُ الصورةِ الجديدةِ — لا فقدَ بياناتٍ. والنشرُ **مقرونٌ** لأنَّ المعالجَ الذي يُغني الحمولةَ يُشحَنُ مع المخطّطِ.",
    breaksPreviousRelease: false,
    rollbackPath: "code-only",
    coupledDeploy: true,
    owner: "منفّذ المستودع",
  ...
  },
  {
    migration: "20260918030000_f12_07_lost_item_mechanism.sql",
    change: "revoke_function:open_support_ticket(5)",
    why: "الهجرةُ `20260918030000` (`F12-07`) تُعيدُ تعريفَ `open_support_ticket(uuid, bigint, support_ticket_type, text, uuid)` بـ`create or replace` **بالتوقيعِ والمردِّ ومفاتيحِه نفسِها** لتُودِعَ صفَّ بلاغِ المفقودِ في `notification_outbox` (`kind='lost_item_report'`، `dedup_key='lost_item:'||ticket_id`) **مشروطاً** بطلبٍ مُسنَدِ السائقِ مُكمَّلٍ: لا يُودَعُ البلاغُ إلّا حينَ `p_type='lost_item'` و`assigned_driver_id is not null` و`status='completed'`. وبهذا **يُحفَظُ عقدُ `F3-08` المحفوظُ (`ح-8`)**: تذكرةُ المفقودِ تُفتَحُ بلا طلبٍ (ok:true) كما كانت، وطلبٌ بلا سائقٍ مُسنَدٍ أو غيرُ مُكمَّلٍ يُفتَحُ تذكرتُه بلا بلاغٍ فلا صفٌّ ميّتٌ يُحاولُ التسليمَ إلى لا أحدَ. وحارسُ مِلكيّةِ الطلبِ (`ORDER_NOT_YOURS`) والتهدئةُ وقفلُ الصفِّ وتوليدُ المرجعِ وسجلُّ التدقيقِ منسوخةٌ حرفاً بحرفٍ عن `20260916020000` ولم يُمَسَّ مفتاحٌ. والسحبُ بعدَه **إعادةُ قفلِ السطحِ كما كانَ لا تضييقٌ جديدٌ**: الدالّةُ كانت لـ`service_role` وحدَها قبلَ التغييرِ وتبقى كذلكَ بعدَه، و`create or replace` لا تُسقِطُ منحاً قائماً — فالسطرُ تكرارٌ مقصودٌ ليكفيَ الملفُّ بذاتِه إن أُعيدَ بناءُ القاعدةِ. **ولا كسرَ لنسخةٍ سابقةٍ**: الوسائطُ الخمسةُ وأنواعُها كما كانت، والمردُّ بمفاتيحِه ورموزِ رفضِه نفسِها، وكلُّ مسارٍ كانَ يُفتَحُ تذكرةً بالأمسِ يفتحُها اليومَ بالحكمِ نفسِه — فالفرعُ الجديدُ يُضيفُ إيداعاً مشروطاً لا يُسقِطُ قبولاً قائماً. **والنشرُ مقرونٌ في الاتجاهَينِ**: المعالجُ الذي يُسلِّمُ `lost_item_report` (وإغناؤُه بمحادثةِ السائقِ ولغتِهِ) يُشحَنُ مع المخطّطِ، فلا يُنشَرُ أحدُهما دونَ الآخر — صفٌّ بلا معالجٍ لا يُسلَّمُ، ومعالجٌ بلا صفٍّ لا يُلتقَطُ. **والعودةُ بالصورةِ السابقةِ تكفي**: أقصى أثرِها أنَّ بلاغَ المفقودِ يعودُ لا يُودَعُ صفٌّ له في الصادرِ — وذاكَ عيبُ إخبارٍ لا فقدُ بياناتٍ، والتذكرةُ تُفتَحُ كما كانت.",
    breaksPreviousRelease: false,
    rollbackPath: "code-only",
    coupledDeploy: true,
    owner: "منفّذ المستودع",
  ...
  {
    migration: "20260921000000_pd_021_decision_reason.sql",
    change: "drop_function:resolve_safety_incident(3)",
    why: "الهجرةُ تُسقِطُ الدالّةَ القديمةَ (٣ معاملاتٍ) وتُنشِئُ جديدةً (٤ معاملاتٍ) في الملفِّ نفسِه. والدالّةُ الجديدةُ تُلزِمُ سببًا داخليًّا عندَ الإغلاقِ — لا مسارَ قديمَ يُغلقُ بلا سببٍ. والعودةُ بالكودِ وحدَه: الصورةُ السابقةُ تنادي بـ٣ معاملاتٍ فلا تُنشَرُ الهجرةُ دونَ كودِها ولا يُرجَعُ الكودُ دونَ إرجاعِ الدالّةِ معه.",
    breaksPreviousRelease: false,
    rollbackPath: "code-only",
    coupledDeploy: true,
    owner: "منفّذ المستودع",
  ...
  },
  {
    migration: "20260921000000_pd_021_decision_reason.sql",
    change: "revoke_function:claim_notification_delivery(0)",
    why: "الهجرةُ تُعيدُ تعريفَ `claim_notification_delivery` بـ`create or replace` لتُضيفَ فرعَي إثراءِ إشعاراتِ المآلِ العامِّ للمُبلِّغِ، وبقيةُ الفروعِ منسوخةٌ حرفًا بحرفٍ. والسحبُ بعدَه إعادةُ قفلِ السطحِ كما كان — الدالّةُ كانت ممنوحةً لـ`service_role` وحدَها قبلَ التغييرِ وتبقى كذلك بعدَه، و`create or replace` لا تُسقِطُ منحًا قائمًا أصلًا. والعودةُ بالكودِ وحده: الصورةُ السابقةُ لا تعرفُ النوعَينِ الجديدَينِ فلا تُنشَرُ الهجرةُ دونَ كودِها ولا يُرجَعُ الكودُ دونَ إرجاعِ الدالّةِ معه.",
    breaksPreviousRelease: false,
    rollbackPath: "code-only",
    coupledDeploy: true,
    owner: "منفّذ المستودع",
  ...
  {
    migration: "20260921000001_pd_021_contract.sql",
    change: "revoke_function:notification_kind_priority(1)",
    why: "الهجرةُ تُعيدُ تعريفَ `notification_kind_priority` بـ`create or replace` لتُضيفَ فرعَي النوعَينِ الجديدَينِ (رتبةٌ متوسّطةٌ ٣)، وبقيةُ الفروعِ منسوخةٌ حرفًا بحرفٍ. والسحبُ بعدَه إعادةُ قفلِ السطحِ كما كان — الدالّةُ كانت ممنوحةً لـ`service_role` وحدَها قبلَ التغييرِ وتبقى كذلك بعدَه، و`create or replace` لا تُسقِطُ منحًا قائمًا أصلًا. والعودةُ بالكودِ وحده: الصورةُ السابقةُ لا تعرفُ النوعَينِ الجديدَينِ فلا تُنشَرُ الهجرةُ دونَ كودِها ولا يُرجَعُ الكودُ دونَ إرجاعِ الدالّةِ معه.",
    breaksPreviousRelease: false,
    rollbackPath: "code-only",
    coupledDeploy: true,
    owner: "منفّذ المستودع",

...
export const SECURITY_CONTROLS: readonly SecurityControl[] = [
  ...
  },
  {
    ...
    state: "partial",
    rationale:
      "وسيطانِ منفصلانِ قائمانِ فعلاً (`apps/gateway/src/public/security-headers.ts` و`admin/security-headers.ts`) بسياسةِ محتوىً بـ`nonce` ومنعِ تأطيرٍ وفهرسةٍ وكتمِ مُحيلٍ. **والفجوةُ بعينِها**: لا حاجزَ آليَّ يمنعُ صفحةً عامّةً جديدةً من أن تُركَّبَ بلا الوسيطِ، ولا `Strict-Transport-Security` في المستودَعِ (يُنهيهِ الوسيطُ العكسيُّ — بنيةٌ تحتيّةٌ لا شيفرةٌ).",
    evidence: null,
    guard: null,
    owner: "منفّذ المستودع",
    blockedBy: null,
  },
  {
    ...
    state: "partial",
    rationale:
      "نافذةٌ ثابتةٌ قائمةٌ (`apps/gateway/src/rate-limit/fixed-window.ts`) مُركَّبةٌ على الويبهوكِ وموقعِ السائقِ. **والفجوةُ بعينِها**: لا سِجلَّ يقولُ أيُّ مسارٍ يجبُ أن يكونَ محدوداً، فمسارٌ جديدٌ يُولَدُ بلا حدٍّ ولا شيءَ يكشفُه؛ والعدُّ في ذاكرةِ المثيلِ فلا يصمدُ لمثيلَينِ.",
    evidence: null,
    guard: null,
    owner: "منفّذ المستودع",
    blockedBy: null,
  ...
  },
  {
    ...
    state: "partial",
    rationale:
      '`createAdminGuard` على `app.use("*")` يغطّي مساراتِ اللوحةِ كلَّها، ورمزُ `CSRF` قائمٌ في `admin/auth.ts`. **والفجوةُ بعينِها**: الحارسُ مقيسٌ بوجودِه نصّاً في حاجزِ `SEC-01` لا بسلوكِه على قاعدةٍ — لا اختبارَ تكاملٍ يُنادي مساراً إداريّاً بلا جلسةٍ ويقيسُ الرفضَ، ولا يُقاسُ رفضُ رمزِ `CSRF` المزوَّرِ.',
    evidence: null,
    guard: null,
    owner: "منفّذ المستودع",
    blockedBy: null,
  ...
  },
  {
    ...
    state: "partial",
    rationale:
      "جدولُ `audit_log` قائمٌ بتسعةِ أعمدةٍ ويُكتَبُ فيهِ فعلاً. **والفجوةُ بعينِها**: لا سِجلَّ مغلقٌ يقولُ **أيُّ** الأعمالِ يجبُ أن تُدقَّقَ، فعملٌ حسّاسٌ جديدٌ يمرُّ بلا أثرٍ ولا شيءَ يكشفُه — والتدقيقُ الناقصُ أسوأُ من غيابِه لأنَّهُ يُوهِمُ تغطيةً.",
    evidence: null,
    guard: null,
    owner: "منفّذ المستودع",
    blockedBy: null,

...
export const SECURITY_CONTROLS: readonly SecurityControl[] = [
  ...
  },
  {
    ...
    // زيادةٌ (`ح-8`) — 2026-09-20: الحيثيّةُ أدناهُ كانَت تحملُ فجوتَينِ، وآخِرُ
    // جملتِها حرفاً: «ولا فحصَ ثغراتٍ آليٌّ على التبعيّاتِ». **وتلكَ الجملةُ
    // سُدَّت** بـ`ADR 0150`: حاجزٌ في سلسلةِ `ci` وثلاثُ خطواتٍ مُسمّاةٍ في
    // `verify` يُشغِّلُ `bun audit` ويُسقِطُ البناءَ على نشرةٍ عندَ حدِّ
    // `moderate` بلا إقرارٍ حيٍّ، ويسقطُ مغلقاً متى تعذَّرَ القياسُ. وأوّلُ
    // قياسٍ كشفَ **ثلاثَ نشراتٍ متوسّطةٍ** على `hono@4.13.0` — منها استنزافُ
    // ذاكرةٍ في `parseBody()` — فعُولِجَت بالرفعِ إلى `^4.13.5` لا بالإقرارِ.
    // **والحالُ يبقى `partial`** لأنَّ الفجوةَ الأولى (طزاجةُ العقودِ) باقيةٌ
    // بـ`O-6`، والدعوى القديمةُ محفوظةٌ ههنا لا ممحوّةٌ.
    // الدليلُ: `docs/evidence/security/SEC-15-DEPENDENCY-ADVISORIES-20260920.md`.
    state: "partial",
    rationale:
      "العقودُ المنقولةُ من `CORE` مُثبَّتةٌ ببصمةِ بايتاتٍ يحرسُها حاجزانِ. **والفجوةُ الباقيةُ بعينِها**: طزاجتُها **غيرُ قابلةٍ للتحقُّقِ في CI** — `uxxxug/wasla-core` مستودَعٌ خاصٌّ ورِمزُ جرياتِنا لا يقرؤُه؛ وذاكَ `O-6`، قرارُ مالِكٍ لا شغلُ وكيلٍ. **وأمّا فحصُ الثغراتِ الآليُّ فقد بُنيَ** (`ADR 0150`): `bun audit` في سلسلةِ `ci` بحَكَمٍ نقيٍّ وسِجلِّ إقراراتٍ مغلقٍ مؤقَّتٍ بتاريخِ انتهاءٍ، وتعذُّرُ القياسِ إخفاقٌ لا خُضرةٌ — ولا يُدَّعى بهِ أمنُ التبعيّاتِ بل «لا نشرةَ معروفةً فوقَ الحدِّ اليومَ».",
    evidence: null,
    // زيادةٌ (`ح-8`) — 2026-09-20: قياسُ الشقِّ المسدودِ من الفجوةِ. و`evidence`
    // يبقى `null` عن قصدٍ: دعوى البناءِ لا تُشترى بشقٍّ، والحالُ `partial`.
    gapMeasurement: "docs/evidence/security/SEC-15-DEPENDENCY-ADVISORIES-20260920.md",

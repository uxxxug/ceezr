...
export const DECLARED_EXTRA_BUNDLES: Readonly<Record<string, string>> = {
  ...
  /**
   * `D-29` · `ADR 0186`: قاموسا الإنجليزيّةِ والأرديّةِ خرجا من `shell` (≈ 52.7 KB مضغوطةً على
   * المسارِ الحرجِ لكلِّ مستخدمٍ) إلى حزمتَينِ مؤجَّلتَينِ تُنزَّلانِ لمن لغةُ حسابِه إحداهما.
   * وحاجزُ `apps/miniapp/vite/assert-initial-dictionaries.ts` يمنعُ عودتَهما إلى الحِملِ الأوّلِ.
   */
  en: "قاموسُ الإنجليزيّةِ للتطبيقِ المصغَّرِ — مؤجَّلٌ يُنزَّلُ لمن لغةُ حسابِه `en` وحدَه (D-29 · ADR 0186)",
  /**
   * `D-32`: سجلُّ الرحلاتِ (`SR-09`/`SR-10`) ومركزُ الإشعاراتِ (`SS-07`) خرجا من `rider-home` — جدولُ 9.4 يحصرُها في
   * «الرئيسية، التسعير، اختيار الخدمة» ولا يُسمّي حزمةً للسجلِّ. وحاجزُ `assert-rider-first-surface` يمنعُ عودتَهما.
   */
  "rider-history":
    "سجلُّ الرحلاتِ وتفاصيلُها ومركزُ الإشعاراتِ للراكبِ — مؤجَّلٌ بطلبِه، خارجَ محتوى `rider-home` في 9.4 (D-32)",
  ur: "قاموسُ الأرديّةِ للتطبيقِ المصغَّرِ — مؤجَّلٌ يُنزَّلُ لمن لغةُ حسابِه `ur` وحدَه (D-29 · ADR 0186)",
};

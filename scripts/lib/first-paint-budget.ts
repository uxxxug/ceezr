 ...
 *   **وبقرارِ المالكِ `DEC-19` (2026-09-24 · `ADR 0185`)**: شاشةُ «افتح من تيليجرام» التي
 *   يقيسُها هذا الحَكَمُ **ليست مسارَ قبولٍ** للصفوفِ 3–5؛ ودورُه شرطُ حياةٍ (حاجزٌ) وحارسُ
 *   انحدارٍ على «3G» (السقفُ 9,200 ms). والحكمُ على سطحِ الراكبِ في `rider-surface-budget.ts`.
 ...
 *
 * لماذا سجلُّ خروقٍ مُعلَنةٍ لا بوّابةٌ حمراءُ دائمةٌ (`DEC-19`): ملفُّ Chromium «3G»
 * (كانَ اسمُه «Slow 3G») يُضيفُ إلى **كلِّ طلبٍ** تأخيراً مُحاكىً 2000 ms — يمثِّلُ شبكةً زمنُ
 * رحلتِها 400 ms × 5 — فيجعلُ `FCP ≤ 1.8s` **مستحيلاً تحتَ هذا الملفِّ المُحاكى**: رؤوسُ
 * المستندِ لا تصلُ قبلَ 2000 ms — وسقفُ الحملِ الأوّلِ في الجدولِ نفسِه (180 KB = 184,320
 * بايتاً) يحتاجُ 3.69 ثانيةً للنقلِ وحدَه على 50,000 بايتٍ/ث. (صُحِّحَت هذهِ الجملُ
 * 2026-09-24 · `ح-8` — نصُّها السابقُ محفوظٌ في
 * `docs/evidence/architecture/F1-09-20260924-dec19-comparison.md` §١.) فالعقدُ
...
export const DECLARED_BREACHES: readonly DeclaredBreach[] = [
  {
    ...
    decision: "DEC-19",
    reason:
      "حارسُ انحدارٍ لا مسارُ قبولٍ (DEC-19): لا رسمَ قبلَ تنزيلِ الحملِ الأوّلِ وتنفيذِه (163,374 بايتاً بعدَ الضغطِ على 50,000 بايتٍ/ث)، وتأخيرُ طلبِ المستندِ المُحاكى (2000 ms) وحدَه يتجاوزُ الحدَّ تحتَ هذا الملفِّ",
  },
  {
    profileId: SLOW_3G.id,
    metric: "lcp",

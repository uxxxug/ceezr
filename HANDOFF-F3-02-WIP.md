...
| ٦ تكاملُ `F3-02` وحدَه | `TEST_DATABASE_URL=… bun test tests/integration/driver-offers.test.ts` | **أخضرُ** · **`24 pass · 0 fail`** · 73 توكيداً · على PostgreSQL 18 + PostGIS حقيقيٍّ. ومنها الحالةُ ٢٢: **قبولانِ متزامنانِ يُنجِحانِ واحداً فقط** · والحالةُ ٦: `seconds_left` يتناقصُ بساعةِ القاعدةِ بينَ قراءتَينِ |
| ٧ حزمةُ التكاملِ كاملةً | `bun run test:integration` | **ليسَ أخضرَ محلّيّاً** · `916 pass · 5 fail · 1 error` · 921 حالةً في 98 ملفّاً · 523 ثانيةً — انظُرْ §٠-ب |

وما بعدَ المرحلةِ السابعةِ: انظُرْ §٥ — ما لم يُعلَنْ حكمُه ههنا **لم يُشغَّلْ**.

### ٠-ب) الخمسةُ الساقطةُ في الحزمةِ الكاملةِ — موقوفٌ عندَها

الساقطاتُ: `F7-07` المُطبّقُ الآمنُ (مهلةُ خُطّافٍ 5000مث) ·
`CAP-003` الجوارُ على PostGIS ×2 (مهلةُ خُطّافٍ) · `F7-02` خطّةُ التنفيذِ
والفهرسُ · وخُطّافٌ واحدٌ غيرُ مُسمّى (120 ثانيةً).

**ولا واحدٌ منها في ملفٍّ مسّه `F3-02`**: `git diff main...HEAD` على تلكَ
الملفّاتِ الثلاثةِ **خالٍ تماماً**.

وأُعيدَ تشغيلُها على **`main` نقيّاً** في شجرةِ عملٍ مستقلّةٍ (`727a87b`، لا
أثرَ لـ`F3-02` فيها): فسقطَ `F7-02` **بالنصِّ والمدّةِ عينِها** (606مث)
— فهوَ **موروثٌ قطعاً لا مُحدَثٌ**. والأربعةُ الباقيةُ **نجحَت منفردةً**
على `main`، فهي مهلاتُ خُطّافٍ تظهرُ تحتَ ضغطِ الحزمةِ الكاملةِ في صندوقِ
PG18 المحلّيّ، لا أحكامُ رمزٍ.

**ولم يُمسَّ منها اختبارٌ واحدٌ**: لا تعطيلَ، ولا تخفيفَ، ولا تمديدَ مهلةٍ،
ولا إدخالَ في سجلِّ تجاوزٍ، ولا نقلَ للمشكلةِ. وحكمُ CI على PG17 هوَ
الفَيصلُ ولم يُستبدَلْ بأخضرَ محلّيٍّ.

ومقارنةُ خطِّ أساسٍ كاملةٌ على `main` **قيدَ التشغيلِ** (§٠-ج).

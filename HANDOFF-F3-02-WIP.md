...
### ٠-ج) مقارنةُ خطِّ الأساسِ — الفرعُ **أنقى منَ `main`** في الصندوقِ عينِه

شُغّلَت الحزمةُ الكاملةُ مرّتَينِ على القاعدةِ والماكينةِ نفسِهما:

| الشجرةُ | المُقاسُ | الساقطُ |
|---|---|---|
| **`main` نقيّاً** (`727a87b`، شجرةُ عملٍ مستقلّةٌ) | 892 حالةً · 97 ملفّاً · 472ث | `880 pass ·` **`12 fail`** |
| `feature/wasla-build-wip` | 921 حالةً · 98 ملفّاً · 523ث | `916 pass ·` **`5 fail`** |

**والخمسةُ الساقطةُ على الفرعِ جُزءٌ محضٌ منَ الاثنتَي عشرةَ الساقطةِ على
`main`** — كلُّ واحدةٍ منها (`F7-07` · `CAP-003` · `F7-02` · الخُطّافُ غيرُ
المُسمّى) تظهرُ على `main` أيضاً. **فلا إخفاقَ واحداً أحدثَه `F3-02`، بل
أضافَ ملفّاً و٣٦ حالةً ناجحةً (916 − 880) وأسقطَ عددَ الساقطاتِ من 12 إلى 5**
(والفرقُ في العددِ ذاتُه بُرهانُ أنَّها مهلاتُ خُطّافٍ راجفةٌ تحتَ الضغطِ
لا أحكامُ رمزٍ ثابتةٌ).

ومعَ ذلكَ: **الأخضرُ المحلّيُّ لا يقومُ مقامَ حكمِ CI**، ولا الأحمرُ
المحلّيُّ يُسقِطُ حكمَ CI. فالفَيصلُ جولةُ CI على PG17.
...
### أ) ما كانَ موعوداً ههنا — **شُغّلَ كلُّه واخضرَّ**

> تصحيحٌ بالإضافةِ (`ح-8`): كانَ عنوانُ هذا القسمِ «لم يُشغَّلْ شيءٌ من
> هذهِ»، وقد شُغّلَت كلُّها فاخضرَّت — وأحكامُها بأرقامِها في §٠. والقائمةُ
> تُتركُ لإعادةِ القياسِ.

والهجرةُ `20260915020000` **مُطبَّقةٌ على قاعدةِ الصندوقِ المحلّيِّ الزائلةِ
وحدَها** (لا قاعدةَ حقيقيّةَ ولا مشتركةَ)، ودوالُها الثلاثُ تُقرأُ منَ
الكاتالوجِ (`driver_offer_board` · `driver_offer_detail` · `driver_accept_offer`)
وتُنادى فعلاً في ٢٤ حالةَ تكاملٍ ناجحةً.
...
### ب) ما لم يُنجَزْ من دورةِ العملِ — **وههنا وقفَ العملُ بالدقيقِ**

أُنجِزَ منها بعدَ كتابةِ هذا القسمِ (`ح-8`):

- ✓ قياسُ «قبلَ/بعدَ» لحزمةِ التكاملِ — **فُعِلَ** (§٠-ج): `main` 12 ساقطةً
  مقابلَ 5 على الفرعِ، والخمسةُ جُزءٌ منها — **فلا انحدارَ**.
- ✓ تدقيقُ دعاوى `ADR 0117` مقابلَ `ح-5` — **صُدّقَت بتشغيلٍ فعليٍّ**:
  ٢٤ حالةَ تكاملٍ و٤١ حالةَ وحدةٍ وثمانُ قواعدَ لكلٍّ `describe` مُزروعٌ (`ح-7`).
- ✓ تدقيقُ `ROADMAP.md` و`ROADMAP-MASTER.md` مقابلَ `ح-4` — `F3-02` على `[~]`
  لا `[x]`، والشرطُ منصوصٌ في السطرِ نفسِه.
- ✓ التزامُ الاختبارِ الجديدِ بـ`OPS-019` — الحاجزُ يراهُ ضمنَ 97 ملفّاً، ولا
  إعفاءَ أُضيفَ لهُ.

**والباقي الذي وقفَ عندَه العملُ — بندٌ واحدٌ لا غيرُ**:

**حُكمُ CI على PG17 لم يُتلَ بعدُ.** وموضِعُ الحُبسةِ مُقاسٌ:
`.github/workflows/ci.yml` يُشغَّلُ على `push` لـ`[main, "feat/**"]` أو على
`pull_request` وحدَهما. واسمُ هذا الفرعِ `feature/…` **لا يوافقُ
`feat/**`**، فلا جولةَ CI تنعقدُ بدفعٍ إليه. فالطريقُ إلى الحُكمِ
واحدٌ من اثنَينِ، وكلاهُما يحتاجُ إذنَ المالكِ:

1. فتحُ طلبِ دمجٍ (فيفتحُ `pull_request` الجولةَ) — والمالكُ منعَ فتحَه
   قبلَ تمامِ الخُضرةِ.
2. دفعُ الالتزاماتِ عينِها إلى اسمٍ يوافقُ `feat/**` (لا طلبَ دمجٍ ولا
   دمجَ) — والمالكُ ألزمَ العملَ بـ`feature/wasla-build-wip` وحدَه.

**والمأزقُ لا يُحَلُّ محليّاً**: تمامُ الخُضرةِ محليّاً غيرُ مدرَكٍ في هذا
الصندوقِ، لأنَّ `main` نفسَه يُسقِطُ 12 حالةً فيه وهوَ أخضرُ في CI
(`898 pass · 0 fail`). فالقيدُ محليٌّ لا رمزيٌّ، ولا يُداوى بتخفيفِ
اختبارٍ (ولم يُخفَّفْ).

### ب-١) الخطوةُ التاليةُ بعينِها

إن أذِنَ المالكُ بأحدِ الطريقَينِ: تُنتزَعُ جولةُ CI، ويُقرأُ حكمُها
كاملاً (`verify` · التكاملُ على PG17 · `real-redis` · `chaos` · `roadmap`).
ولا دمجَ ألبتّةَ ما لم تكنِ الجولةُ **خضراءَ بلا استثناءٍ**. ولا `[x]`
في الخارطةِ حتّى **ثلاثِ جولاتٍ خضراءَ متتاليةٍ** (`ح-4`).

والنقطةُ الحاليةُ **مستقرّةٌ وآمنةٌ**: لا هجرةَ في منتصفِ تطبيقٍ، ولا
دمجَ معلّقاً، ولا شجرةَ عملٍ معلّقةً (أُزيلَت)، والشجرةُ نقيّةٌ
ومدفوعةٌ إلى `origin/feature/wasla-build-wip`.

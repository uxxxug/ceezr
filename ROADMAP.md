...
للصفوفِ والكُتَلِ؛ (٢) الطابورُ — `notification_outbox` و`order_offers`؛ (٣)
`Redis` — عميلٌ مُحقونٌ معدودٌ؛ (٤) النقلُ الشبكيُّ — بايتاتُ ردودِ `HTTP`؛
(٥) التخزينُ — صفوفٌ مُدخَلةٌ. ولا تُقاسُ النسخُ (Replication) — `WAL`/`LSN`
غيرُ مستقرٍّ في CI.

**المقيسُ:** 14887 صفّاً من سقفِ 15100 · 10693 كتلةً من سقفِ 10872 · 7 رسائلِ طابورٍ من
سقفِ 7 · 122 أمرَ `Redis` من سقفِ 160 · 11862 بايتاً من سقفِ 524288 · 20 صفّاً من
سقفِ 20.
...
· حاجزٌ ساكنٌ بـ10 قواعدَ يحرسُ بقاءَ القياسِ
(`scripts/check-resource-usage-budget.ts`) · سالبةٌ مبذورةٌ لكلِّ قاعدةٍ من
التسعَ عشرةَ (`ح-7`) · ستُّ خطواتٍ مُسمّاةٍ في `CI`.

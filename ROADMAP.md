...
| **الطورُ الجاريُ 2026-09-22 — قياسُ دالّةِ ومحوّلِ التعذُّرِ (الخطوةُ الخامسة)** | `tests/integration/notification-outbox-mark-undeliverable.test.ts` (٧ اختباراتٍ) يقيسُ الوصلَ بينَ الكودِ والقاعدةِ: (١) `mark_notification_undeliverable` برمزِ حجزٍ مطابِقٍ تُعلِنُ `undeliverable` بـ`died_at` و`dead_reason` وتُصفّي `claim_token`، (٢) رمزُ حجزٍ خاطئٌ ⇒ `ok=false` ولا تغييرَ، (٣) صفٌّ ليسَ في `sending` ⇒ `ok=false`، (٤) `NotificationOutboxAdapter.undeliverable` ينادي الدالّةَ ويُعيدُ النتيجةَ، (٥) `deliverNotification` بمعالجٍ يُرجِعُ `undeliverable: true` ⇒ الصفُّ `undeliverable` لا `dead`، (٦) معالجٌ ناجحٌ ⇒ `delivered` لا `undeliverable`، (٧) `deliverNotificationBatch` لا ينكسرُ بعدَ صفٍّ `undeliverable`. مُسجَّلٌ في `scripts/lib/skip-registry.ts` (TEST_DATABASE_URL). **ولا يُقاسُ `telegram_id = null` في القاعدةِ** — العمودُ `not null`، وحرسُ `String(null)` يُقاسُ وحدويًّا. |

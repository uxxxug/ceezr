...
### Reservation `D-26` — **البوّابةُ بلا رؤوسِ CORS: كلُّ راكبٍ يرى «لا اتصال» في طوبولوجيا النشرِ المُعلَنةِ** (opened 2026-09-24, before any file was edited · مُكتشَفٌ أثناءَ حجزِ `F1-09` الصفِّ الخامسِ)

| الحقلُ | القيمةُ |
|---|---|
| البندُ | `D-26` (مُكتشَفٌ · سجلُّ الديونِ) — يحجبُ `F1-09` الصفَّ 5 ويمسُّ `F1-10` · `TG-005` |
| الفجوةُ | `ADR 0165` يضعُ التطبيقَ موقعاً ساكناً (`waslah-miniapp`) والبوّابةَ خدمةً أخرى، و`VITE_WASLAH_API_BASE` عنوانُها — **أصلانِ**. والبوّابةُ لا تُصدِرُ `Access-Control-Allow-Origin` لأيِّ مسارِ HTTP (مقروءٌ: لا `cors` إلّا لـSocket.IO في `apps/gateway/src/index.ts`). **مقيسٌ بمتصفّحٍ حقيقيٍّ** على بوّابةٍ حقيقيّةٍ وقاعدةٍ حقيقيّةٍ: `OPTIONS /v1/session/telegram` ← `404` و`PreflightMissingAllowOriginHeader`، و`/health` ← `MissingAllowOriginHeader`، والشاشةُ «لا اتصال». وفراغُ `VITE_WASLAH_API_BASE` («نفسُ الأصلِ») لا يُنقِذُ: الموقعُ الساكنُ لا API فيه |
| النطاقُ المحجوزُ | وسيطُ CORS في البوّابةِ **مقصورٌ على مساراتِ التطبيقِ** (`/v1/*` · `/health`) بلا بيانِ اعتمادٍ · رؤوسُ الطلبِ المُرسَلةُ والمقروءةُ (`x-request-id` · `retry-after`) · `Access-Control-Max-Age` · اختبارُ وحدةٍ بسوالبَ (المسارُ الإداريُّ والويبهوكُ بلا رؤوسٍ) · حارسٌ يُسقِطُ البناءَ إن نادى التطبيقُ مساراً لا يغطّيه الوسيطُ أو أرسلَ رأساً لا يُجيزُه · قياسُ المتصفّحِ نفسُه برهاناً |
| ما لا يُمَسُّ | `/admin` · ويبهوكُ تيليجرام والدفعِ · استقبالُ أحداثِ CORE · سياسةُ `connect-src` · لا تبعيّةَ جديدةَ (`hono/cors` من `hono` القائمةِ) · لا نشرَ |

### Reservation `F1-09` (الصفُّ الخامسُ) — **وقتُ التفاعلِ بعدَ فتحِ تيليجرام: حدٌّ في جدولٍ لا يقيسُه شيءٌ** (opened 2026-09-24, before any file was edited)

| الحقلُ | القيمةُ |
|---|---|
| البندُ | `F1-09` (`docs/ROADMAP-MASTER.md`)، الحالةُ `[~]` · الصفُّ 5 من القسمِ 9.9: «وقتُ التفاعلِ بعدَ فتحِ تيليجرام ≤ 2.0s» |
| الفجوةُ | وظيفةُ `browser-first-paint` (`ADR 0183`) تقيسُ شاشةَ «خارجَ تيليجرام» وحدَها — لا `initData` ولا جلسةَ ولا سطحَ دورٍ. فالصفُّ 5 لا يقيسُه شيءٌ |
| النطاقُ المحجوزُ | استخراجُ عُدّةِ المتصفّحِ من `scripts/measure-first-paint.ts` إلى مكتبةٍ مشتركةٍ بلا تغييرِ سلوكٍ · قياسٌ في وظيفةِ التكاملِ (PostgreSQL حقيقيّةٌ): بوّابةٌ حقيقيّةٌ + `initData` موقَّعةٌ برمزِ اختبارٍ + مضيفُ تيليجرام مُصطنَعٌ مُعلَنٌ · حَكَمٌ نقيٌّ بسوالبَ · خرقٌ يُحصَرُ بـ`DEC-19` إن وقعَ |
| ما لا يُمَسُّ | نصُّ `F1-09` وحدودُ 9.9 (`ح-1`) · حاجزُ `browser-first-paint` وقواعدُه · لا تبعيّةَ جديدةَ · لا نشرَ |

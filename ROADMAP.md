...
### Reservation `SEC-20` — **مسارُ استردادِ حسابٍ بمراجعةٍ إداريّةٍ صريحةٍ وسجلِّ قرارٍ كاملٍ** (opened 2026-09-22)
...
| النطاقُ المحجوزُ | `supabase/migrations/20260923000000_sec_20_account_recovery.sql` · `apps/gateway/src/routes/admin-ui.ts` (مسارُ `POST /admin/users/:id/recovery` و`POST /admin/recovery/:requestId/review` و`GET /admin/recovery`) · `apps/gateway/src/admin/queries.ts` · `packages/infrastructure/db/schema-contract.ts` · `scripts/lib/audit-actions-registry.ts` · `scripts/lib/skip-registry.ts` · `apps/gateway/src/rate-limit/policy.ts` · `apps/gateway/src/object-authorization-contract.ts` · اختباراتُ تكاملٍ على PostgreSQL حقيقيٍّ · اختباراتُ وحدةٍ · `docs/evidence/security/SEC-20-account-recovery-20260922.md` · ملاحقُ هذا الملفِّ بالذيلِ |

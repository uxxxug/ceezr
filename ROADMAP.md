...
| الحقلُ | القيمةُ |
|---|---|
| المقيسُ قبلَ التنفيذِ | المسارُ **قائمٌ ومُختبَرٌ في CI بلا زمنٍ**: `verifyBackupRestore` يستعيدُ الأرشيفَ في قاعدةً مستقلّةً ويقيسُ تكافؤَ البصمةِ (بنيةٌ · دوالٌّ · سياساتُ RLS · قيودٌ · فهارسٌ · صفوفٌ · أدوارٌ) — لكنَّ **لا ساعةَ** في المسارِ كلِّهِ: لا `pg_dump` ولا `pg_restore` ولا البصمةُ تُقاسُ مدّتُها، ولا `db_backups.restore_verification_detail` تحملُ زمناً، و`scripts/backup-restore-drill.ts` (التمرينُ اليدويُّ) يطبعُ التكافؤَ بلا زمنٍ. والبندُ (`F11-10` · `OPS-008`) يطلبُ «تدريبَ استعادةٍ **موثَّقاً بزمنٍ فعليٍّ**» — فالشقُّ المفقودُ عينُهُ الزمنُ. |
| ما يُبنَى | (١) قياسُ الأطوارِ في `packages/infrastructure/backup/restore-verifier.ts`: `sourceFingerprintMs` · `restoreMs` · `restoredFingerprintMs` بـ`performance.now()`، تُعادُ في `RestoreVerification.timings` (زيادةٌ على الواجهةِ لا كسرٌ لها). (٢) تسجيلُها في `VerifyBackupRestoreOutcome` وفي `db_backups.restore_verification_detail` (عمودُ jsonb قائمٌ — بلا هجرةٍ). (٣) التمرينُ اليدويُّ يقيسُ مدّةَ التفريغِ والكلِّ ويطبعُ الأطوارَ في حاصلِهِ. (٤) حَكَمٌ خالصٌ `scripts/lib/backup-drill-timing.ts` (`timing.recorded` · `timing.source-fingerprint-positive` · `timing.restore-positive` · `timing.restored-fingerprint-positive` · `timing.total-covers-phases` · `timing.detail-persisted`) وسالباتُه المبذورةُ (`ح-7`). (٥) حالةٌ في `tests/integration/backup-restore-verification.test.ts` تُوكِّدُ الأزمنةَ المقيسةَ من استعادةٍ حقيقيّةٍ ومطابقتَها في `db_backups`. (٦) خطوةٌ مُسمّاةٌ في وظيفةِ PostgreSQL تُشغِّلُ التمرينَ فيطبعَ زمنَهُ الفعليَّ في سجلِّ التشغيلِ. |
| الإنفاذُ | خطوةٌ مُسمّاةٌ في وظيفةِ «تكامل على PostgreSQL حقيقي» (التمرينُ بـ`DATABASE_URL` على القاعدةِ التي طبَّقتِ الهجراتِ) · سالبةٌ مبذورةٌ لكلِّ قاعدةٍ (`ح-7`) · حالةُ تكاملٍ ضمنَ ملفِّ التحقُّقِ القائمِ (بوّابةُ `TEST_DATABASE_URL` نفسُها ومُدخلُ سجلِّ التجاوزِ يُحدَّثُ بالزيادةِ) |
| الوثائقُ | `ADR 0201` · دليلٌ في `docs/evidence/recovery/F11-10-20260927-restore-drill-timing.md` بأزمنةٍ فعليّةٍ من تشغيلٍ محلّيٍّ وتشغيلِ CI · زيادةٌ على صفِّ `F11-10` في MASTER (`ح-1`: نصُّ البندِ لا يُمَسُّ) · §25 · `SYSTEM_STATE` |
| ما لا يُدَّعى | **لا قلبَ إلى `[x]` ولا `RTO`/`RPO` مدَّعىً**: `ADR 0047` §٣ أجَّلَ قياسَهُما إلى بيئةٍ شبيهةٍ بالإنتاجِ، والزمنُ المقيسُ ههنا زمنُ **التمرينِ على قاعدةِ حجمِ الهجراتِ** (لا حجمَ إنتاجَ — `B-1`) في عدّاءِ CI، فيُسمّى بما هوَ. ولا دوريّةَ إنتاجٍ تُدَّعى: مهمةُ العاملِ مسجَّلةٌ كلَّ يومٍ في `container.ts` لكن لا عاملَ إنتاجيًّا بعدُ (`DEC-11`)؛ والدوريّةُ المقيسةُ هي إعادةُ التمرينِ مع كلِّ تشغيلِ CI. |
...
| الحقلُ | القيمةُ |
|---|---|
| المقيسُ قبلَ التنفيذِ | مسارا الويبهوك والمراجعةِ يُعلنانِ فشلَ المزوّدِ بصدقٍ (`PROVIDER_LOOKUP_FAILED` بـ503 · عدّادُ `failed` في المراجعةِ) ويُختبرانِ على قاعدةٍ حقيقيّةٍ (`payment-real-flow.test.ts` · `payment-reconciliation.test.ts`) — **لكنْ بمزوّدٍ سليمٍ لا بعطلٍ محقونٍ على السِلكِ**، ومسارُ بدءِ الشحنةِ (`chargeSubscription`) لا يُقاسُ تحتَ عطلٍ قطّ: لا اختبارَ يُطفئُ خادمَ مزوّدٍ حقيقيًّا أثناءَ شحنةٍ، ولا يقيسُ أنَّ الصفَّ المعلَّقَ يبقى بلا فسادٍ ماليٍّ، ولا أنَّ السائقَ يستطيعُ استعادةَ رابطِ دفعٍ بعدَ تعافي المزوّدِ. |
| ما يُبنَى | `tests/integration/payment-provider-outage.test.ts`: خادمُ Tap محليٌّ بأطوارٍ (سليمٌ · مُطفَأٌ · مُعلِّقٌ · 5xx · حمولةٌ فاسدةٌ)، ومعاملةٌ على PostgreSQL حقيقيّةٍ تُدارُ في كلِّ طورٍ: بدءُ الشحنةِ أثناءَ العطلِ (خطأٌ صادقٌ · لا قيدَ في دفترِ الأستاذِ · لا تفعيلَ للاشتراكِ)، والمراجعةُ أثناءَ العطلِ (صفٌّ معلَّقٌ بمرجعٍ عندَ المزوّدِ يبقى معلَّقًا · عدّادُ `failed` يُعلنُه)، والتعافي (المراجعةُ تُسوّي مرّةً واحدةً · قيدٌ واحدٌ في الدفترِ · اشتراكٌ فعّالٌ). والحَكَمُ الخالصُ `scripts/lib/payment-outage.ts` وسالباتُه (`ح-7`). |
| اكتشافٌ أثناءَ الحجزِ (`D-38` — يُسجَّلُ ويُحجَزُ بهذا الصفِّ قبلَ أيِّ تعديلٍ) | **فشلُ بدءِ الشحنةِ يسجنُ السائقَ يومَهُ كاملًا بلا سبيلِ استعادةٍ**: `subscribePlan` يُنشئُ الصفَّ `pending` ثمَّ يستدعي `chargeSubscription`؛ فإذا فشلَ (عطلُ المزوّدِ) عادَ بخطأٍ صادقٍ لكنَّ الصفَّ يبقى `pending` بلا مرجعِ مزوّدٍ ولا رابطِ دفعٍ، ومفتاحُ الإيدمبوتنسيّ اليوميُّ (`driver_subscription:{driver}:{plan}:{day}`) يُعيدُ المحاولةَ الثانيةَ إلى الصفِّ نفسِهِ برابطٍ `null` — **نجاحٌ كاذبٌ** لا رابطَ فيهِ، والمراجعةُ لا تغطّيهِ (لا مرجعَ عندَ المزوّدِ يُسألُ به)، فلا يستطيعُ السائقُ الاشتراكَ قبلَ اليومِ التاليِ حتى بعدَ تعافي المزوّدِ. **العلاجُ في الجذرِ**: `packages/application/financial/subscribe-plan.ts` — إعادةُ بدءِ الشحنةِ على الصفِّ المعلَّقِ **نفسِهِ** (بلا صفٍّ ثانٍ ولا مفتاحٍ جديدٍ) متى كانَ `pending` بلا مرجعِ مزوّدٍ ولا رابطٍ محفوظٍ: فالشحنةُ الأولى لم تُسلَّمْ للسائقِ رابطًا قطُّ، والصفُّ الواحدُ يُثبِتُ التفردَ. وسالبةٌ تُثبِتُ سقوطَ التوكيدِ لو عادَ المحاولةُ الثانيةُ برابطٍ `null`. |
| الإنفاذُ | يجري في وظيفةِ «تكامل على PostgreSQL حقيقي» (خطوةٌ مُسمّاةٌ جديدةٌ بـ`TEST_DATABASE_URL`) · سالبةٌ مبذورةٌ لكلِّ قاعدةٍ (`ح-7`) في `tests/unit/payment-outage-judge.test.ts` · مُدخلٌ في `scripts/lib/skip-registry.ts` (يفرضُهُ `check-skip-classification`). |
| الوثائقُ | `ADR 0200` · دليلٌ في `docs/evidence/architecture/F11-07-20260927-payment-provider-outage.md` · زيادةٌ على صفِّ `F11-07` في MASTER (`ح-1`: نصُّ البندِ لا يُمَسُّ) · §25 · `SYSTEM_STATE` |
| ما لا يُدَّعى | لا قلبَ إلى `[x]` (`ح-4`)، ولا «مُثبَتٌ» (`ح-5`): البندُ في القسمِ الحادي عشرَ يُقصَدُ به **مزوّدُ دفعٍ حقيقيٌّ في بيئةٍ شبيهةٍ بالإنتاجِ**؛ والمقيسُ ههنا مزوّدٌ محليٌّ يحاكي عقدَ Tap تحتَ عطلٍ محقونٍ على السِلكِ، لا انقطاعُ Tap نفسِه ولا حِملٌ ولا أموالٌ حقيقيّةٌ. ويُفصَحُ: صفوفٌ `pending` بلا مرجعِ مزوّدٍ (فاتورةُ Moyasar المستضافة) تبقى خارجَ تغطيةِ المراجعةِ كما هو مُعلَنٌ في `reconcile-pending-payments.ts`. |
| المقيسُ محلّيّاً | تكاملُ `payment-provider-outage.test.ts`: **7/7 ناجحةٌ** على PostgreSQL حقيقيّةٍ (201 هجرةً) — الأطوارُ الأربعةُ للشحنةِ كلٌّ في تركيبٍ مستقلٍّ مع تعافٍ حقيقيٍّ يُعيدُ رابطَ `chg_…`، والمراجعةُ تحتَ العطلِ (`examined` = `failed` بنطاقِ `tap`، الصفُّ معلَّقٌ)، والتسويةُ مرّةً واحدةً (قيدٌ واحدٌ · اشتراكٌ واحدٌ · الجولةُ الثانيةُ بلا أثرٍ)، والويبهوكُ 503 بلا حدثٍ. سالباتُ الحَكَمِ: **32/32**. `payment.test.ts`: 15/15 (توقّعانِ كانا يُرمِّزانِ عطبَ `D-38` فصُحِّحا بالإضافةِ). كاملُ الوحداتِ: 6198/0. وملفاتُ الدفعِ الخمسةُ معًا (outage · real-flow · reconciliation · cancel-upgrade · driver-subscription): **49/49**. |
| حكمُ CI (PR #285) | **أخضرُ من الجولةِ الأولى** على `f92cedd8` (تشغيل `36273491606`، 6m34s): الخطوةُ المُسمّاةُ «توقُّفُ مزوّدِ الدفعِ لا يُفسِدُ معاملةً ولا يسجنُ سائقاً (F11-07)» ركضَت بـ`TEST_DATABASE_URL` حقيقيّةٍ — **7/7 ناجحةٌ في 4.18s بلا إخفاقٍ** — ووظيفةُ التكاملِ العامّةُ خضراءُ، وفحصُ تصنيفِ التجاوزِ وتوزيعُ الميزةِ وكاملُ الوظائفِ نجحت. |


### Reservation `F11-04` — **بطءُ PostgreSQL (brownout): مهلةُ استعلامٍ على تجمُّعِ مسارِ الطلبِ تمنعُ الانهيارَ المتتاليَ** (الشقُّ المملوكُ للمستودَعِ · opened 2026-09-25, before any file was edited · **merged 2026-09-25 في PR #277**)
...
| القرارُ | مهلةٌ من **العميلِ** لكلِّ دورٍ في `DB_QUERY_DEADLINE_MS`: 8000ms لـ`gatewayRequest` و`adminRequest` — حدُّ Supabase نفسِه لأدوارِ مسارِ الطلبِ (`authenticated` و`authenticator`: `statement_timeout=8s`، مقروءةً من `pg_roles`؛ ودورُ `postgres` الذي نتّصلُ به `2min`)؛ و`null` مكتوبُ السببِ للعاملِ والقفلِ والتحقُّقِ. `createSql` يلفُّ التجمُّعَ: كلُّ استعلامٍ (وسمٌ · `unsafe` · `file` · داخلَ `begin`/`savepoint`/`reserve`) يُلغى عندَ المهلةِ بـ`57014` ويُنادى خطّافُ رصدٍ يكتبُ `db.query_deadline_exceeded`. والحارسُ `check-connection-budget` يُلزِمُ كلَّ موضعِ إنشاءٍ دورُه ذو مهلةٍ بتمريرِها من الميزانيّةِ (`ADR 0197`). |
| اكتشافٌ أثناءَ التنفيذِ (عولِجَ في النطاقِ) | عبرَ Supavisor **لا يصلُ الإلغاءُ استعلاماً مُخطَّطاً** (pipelined) خلفَ آخرَ على الاتّصالِ نفسِه: مقيسٌ — المُخطَّطُ أكملَ مدّتَه رغمَ إلغاءٍ كلَّ 250ms لستِّ ثوانٍ، وفي وضعِ المعاملةِ `6543` علِقَ؛ ومحلّيّاً يُلغى. و`postgres` يُخطِّطُ متى امتلأت الاتّصالاتُ — أي في البطءِ بعينِه. و`max_pipeline: 0` يقطعُ `onexecute` فيسقطُ `sql.begin` بـ`UNSAFE_TRANSACTION` (مقيسٌ). فالعلاجُ بوّابةٌ بسعةِ السقفِ داخلَ الغلافِ: لا يُسلَّمُ للتجمُّعِ أكثرُ من سقفِه، والزائدُ ينتظرُ حيثُ رفضُه عندَ المهلةِ مضمونٌ، والمعاملةُ تحتجزُ مقعداً طولَها ولاستعلاماتِها بوّابةٌ بسعةِ واحدٍ. وإعادةُ الإلغاءِ كلَّ 250ms لفجوةِ الإلغاءِ. |
| توسيعُ النطاقِ (محجوزٌ بهذا الصفِّ قبلَ تعديلِه) | `packages/infrastructure/db/query-deadline.ts` (الغلافُ والبوّابةُ، مفصولاً عن `client.ts`) · `tests/unit/check-connection-budget.test.ts` (الفحصُ الرابعُ وسالباتُه، وتثبيتاتٌ قديمةٌ تُمرِّرُ المهلةَ الآنَ) · `tests/unit/brownout-invariants.test.ts`. |
| المقيسُ محلّيّاً | بلا مهلةٍ: مسارُ `cities` خُدِمَ بعدَ **3805ms** من بطءٍ 4000ms · العالقُ العشرةُ نجحت كلُّها بعدَ 4006ms · خمسةُ منتظِري قفلٍ على الخادمِ بعدَ 2s — الحَكَمُ: `unrelated.cascaded` · `deadline.absent` · `server.still_waiting`. بمهلةِ 1000ms: `cities` **806ms** · العشرةُ `57014` أطولُها 1015ms (وسمٌ · `unsafe` · `begin`) · صفرُ منتظِرين على الخادمِ · الخطّافُ 10 · التعافي 1ms · صفرُ معاملاتٍ مفتوحةٍ — الحكمُ `ok`. وعبرَ Supavisor (قاعدةُ الاختبارِ، سيدني، RTT ≈ 210ms، مهلةُ 2000ms، سبعةُ `pg_sleep(6)` على خمسةِ اتّصالاتٍ): السبعةُ `57014` أطولُها 2935/2936ms (جلسةً/معاملاتٍ) والتعافي 212/211ms؛ ومسارٌ ثامنٌ أُطلِقَ بعدَ 200ms رُفِضَ **بمهلتِه** عندَ 2000ms لا بعدَ البطءِ (المقاعدُ تتحرّرُ بعدَ المهلةِ ورحلةِ الإلغاءِ). ملفّاتُ التكاملِ التي تبني الحاويتَين (33): 106+125 نجاحاً بلا إخفاقٍ · الوحداتُ 5688/0. |
| توسيعُ النطاقِ بعدَ حكمِ CI الأوّلِ (محجوزٌ بهذا الصفِّ قبلَ تعديلِه) | أسقطَت بوّابةُ التغطيةِ (`OPS-005`) مسارَ «المخطّطُ وتفعيلُ المدن»: 54.82% دونَ 60% — `query-deadline.ts` لا يحمِّله إلّا اختبارُ تكاملٍ لا يدخلُ قياسَ التغطيةِ. العلاجُ في السببِ لا الأرضيّةِ: `tests/unit/query-deadline.test.ts` على تجمُّعٍ مزدوجٍ بشكلِ `Query` في `postgres@3.4.9` يمرُّ بكلِّ فروعِ الغلافِ (إلغاءٌ · إعادتُه · `cancel()` العامُّ · رفضُ المنتظِرِ في البوّابةِ · المعاملةُ ونقطةُ الحفظِ · الحجزُ وإخفاقُه · الشظيّةُ). محلّيّاً: 11/0 والمسارُ 81.42%. |
| حكمُ CI (PR #277) | الأوّلُ على `3646d7d`: `verify` أسقطَته بوّابةُ التغطيةِ (الصفُّ السابقُ) ولم يُسكَت. ثمَّ `36151542083` · `36151547863` على `c9ee232`: السبعُ خضراءُ. وفي وظيفةِ PostgreSQL في الجولتَين: بلا مهلةٍ `cities` 3803/3804ms · العشرةُ نجحت بعدَ 4003/4004ms · خمسةُ منتظِرين — مُدانٌ؛ بمهلةِ 1000ms `cities` 806ms · العشرةُ `57014` أطولُها 1014ms · صفرُ منتظِرين · الخطّافُ 10 · التعافي 1ms — `ok`. |
...
| الحقلُ | القيمةُ |
|---|---|
| المقيسُ | آخرُ تعريفٍ لـ`claim_notification_delivery()` (`20260922050000_sec_19_claim_undeliverable_guard.sql`) يقرأُ `notification_claim_timeout_seconds` بـ`where key = … limit 1` بلا مدينةٍ ولا ترتيبٍ، ثمَّ يسترجعُ صفوفَ كلِّ المدنِ بتلكَ القيمةِ الواحدةِ. الإعدادُ لكلِّ مدينةٍ (`unique (city_id, key)`). وأختُها `claim_subscription_notices(p_city_id)` تقرأُ مهلةَ مدينتِها صحيحاً فلا تُمَسُّ. |
| الأثرُ | مدينتانِ بمهلتينِ مختلفتينِ ⇒ صفُّ إحداهما يُسترجَعُ بمهلةِ الأخرى: إمّا قبلَ أوانِه (عاملٌ حيٌّ يُسلَبُ حجزُه فتتكرّرُ الرسالةُ) أو بعدَه (تأخيرٌ صامتٌ). |
| النطاقُ المحجوزُ | هجرةٌ جديدةٌ تُعيدُ تعريفَ `claim_notification_delivery()` بمهلةِ **مدينةِ الصفِّ** وبقيّتُها منسوخةٌ حرفاً · مُدخلُها في `scripts/lib/rollback-registry.ts` · اختبارُ تكاملٍ بمدينتينِ ومهلتينِ يسقطُ على التعريفِ السابقِ أيّاً كانت القيمةُ المختارةُ · مُدخلُه في `scripts/lib/skip-registry.ts` وعدّادا `tests/unit/skip-audit.test.ts` · `ADR 0196` · دليلٌ · سجلُّ الديونِ · §25 · `SYSTEM_STATE` |
| ما لا يُمَسُّ | `claim_subscription_notices` · قيمُ `platform_settings` · مسارُ التسليمِ وفروعُ حلِّ العنوانِ وحرسُ `SEC-19` · حَكَمُ `F11-02` |
| القرارُ | شرطُ الاسترجاعِ يقرأُ مهلةَ `n.city_id` لكلِّ صفٍّ (افتراضٌ 300 · حدٌّ أدنى 1 كما كانا) في `20260925170000_d_37_claim_timeout_per_row_city.sql` (`ADR 0196`). ولا خطوةَ CI مُسمّاةً: الملفُّ يجري في «اختبارات التكامل على قاعدة حقيقية». |
| المقيسُ محلّيّاً | على التعريفِ السابقِ سقطَت الحالتانِ المتعاكستانِ (الطويلةُ استُرجِعَت في الأولى، والقصيرةُ لم تُسترجَع في الثانية)؛ وعلى الجديدِ نجحتا · ملفّاتُ الصادرِ مع `worker-loss-reclaim`: 68/0 · الإعدادُ أُعيدَ 300 في المدنِ الخمسِ. |
| حكمُ CI (PR #276) | `36142446178` على `cc2e237`: السبعُ خضراءُ. في وظيفةِ PostgreSQL الحالتانِ المتعاكستانِ خضراوانِ: القصيرةُ رمزٌ مُحرَّرٌ والطويلةُ رمزُها القديمُ في كلٍّ منهما. |

### Reservation `F11-02` (الشقُّ المملوكُ للمستودَعِ) — **فقدانُ العاملِ: العملُ يُعادُ استلامُه — مكتوبٌ في الشيفرةِ ومُختبَرٌ بحجزٍ متروكٍ داخلَ العمليّةِ، ولم تُقتَلْ عمليّةُ عاملٍ حقيقيّةٌ قطُّ** (opened 2026-09-25, before any file was edited · **merged 2026-09-25 في PR #275**)

حُجِزَ **قبلَ أوّلِ تعديلِ ملفٍّ**، وفقَ قاعدةِ الحجزِ في `docs/ROADMAP-MASTER.md` §25. فرعُ `feat/f11-02-worker-loss-reclaim` من `main` (`7d959d4`). لا فرعَ ولا PR ولا تنفيذَ سابقٌ يمسُّ `F11-02` (قُرِئَ: `git branch -r` · `gh pr list` · `grep F11-02 docs ROADMAP.md`).

| الحقلُ | القيمةُ |
|---|---|
| المقيسُ قبلَ التنفيذِ | `claim_notification_delivery` يُعيدُ صفَّ `sending` إلى `pending` بعدَ `notification_claim_timeout_seconds`، و`advisory-lock.ts` يَعِدُ في تعليقِه أنَّ «نسخةَ عاملٍ قُتِلَت لا تُخلِّفُ قفلاً أبديّاً». **الدليلُ اختباراتُ تكاملٍ داخلَ عمليّةٍ واحدةٍ** (حجزٌ يُترَكُ بتعديلِ `claimed_at`)؛ لا اختبارَ يقتلُ عمليّةَ عاملٍ حقيقيّةً (`SIGKILL`) وهيَ تحملُ القفلَ أو الحجزَ. |
| ما يُبنى | `tests/integration/worker-loss-reclaim.test.ts` وعمليّةُ عاملٍ فرعيّةٌ (`tests/support/worker-loss-child.ts`) تُشغِّلُ `createJobRunner` بقفلِ `createAdvisoryLock` الحقيقيِّ ومهمّةِ `deliverNotifications` بمنفذِ الصادرِ الحقيقيِّ، وناشرٍ يُرسِلُ إلى خادمِ HTTP مُزيَّفٍ لتيليجرامَ في الأبِ. أطوارٌ: قتلٌ بعدَ الاستلامِ وقبلَ الإرسالِ · قتلٌ بعدَ وصولِ الإرسالِ وقبلَ الإقرارِ · ثمَّ عاملٌ ثانٍ. الحَكَمُ: القفلُ يُحرَّرُ بموتِ الجلسةِ · الصفُّ يبلغُ `sent` مرّةً · رمزُ الحجزِ الميّتِ يُرفَضُ · عدُّ الرسائلِ الواصلةِ لكلِّ صفٍّ يُكتَبُ كما هوَ. |
| الإنفاذُ | يجري في وظيفةِ «تكامل على PostgreSQL حقيقي» (حاجزٌ مانعٌ قائمٌ)؛ وسالبةٌ مبذورةٌ (`ح-7`) تُثبِتُ أنَّ الحَكَمَ يسقطُ على لقطةٍ فاسدةٍ. |
| الوثائقُ | `ADR 0195` · دليلٌ في `docs/evidence/` · زيادةٌ على صفِّ `F11-02` في MASTER (`ح-1`: نصُّ البندِ لا يُمَسُّ) · §25 · `SYSTEM_STATE` |
| توسيعُ النطاقِ (محجوزٌ بهذا الصفِّ قبلَ تعديلِه) | `scripts/lib/worker-loss-invariants.ts` (الحَكَمُ) و`tests/unit/worker-loss-invariants.test.ts` (سالباتٌ) · خطوةٌ مُسمّاةٌ في `.github/workflows/ci.yml` · مُدخلٌ في `scripts/lib/skip-registry.ts` وعدّادا `tests/unit/skip-audit.test.ts` (144 · 1389) — يفرضُها `check-skip-classification`. |
| اكتشافٌ أثناءَ التنفيذِ (سُجِّلَ `D-37` ولم يُمَسَّ) | `claim_notification_delivery` تقرأُ `notification_claim_timeout_seconds` بـ`limit 1` بلا مدينةٍ والإعدادُ لكلِّ مدينةٍ — مهلةٌ غيرُ حتميّةٍ حينَ تختلفُ المدنُ. الاختبارُ يضبطُ **كلَّ** الصفوفِ ثمَّ يُعيدُها فلا يتعلَّقُ حكمُه بالعلّةِ. العلاجُ هجرةٌ تُحجَزُ قبلَ تعديلِها. |
| المقيسُ محلّيّاً | طورُ `/gate`: تحريرُ القفلِ ≤1ms بعدَ خروجِ العمليّةِ · تسلُّمٌ وتسليمٌ 1172ms بعدَ القتلِ (مهلةُ الاختبارِ 2s) · رسالةٌ واحدةٌ من الثاني · محاولتانِ · `delivered` · إعلانُ الميّتِ مرفوضٌ. طورُ `/send`: رسالتانِ (الميّتُ ثمَّ الثاني) والصفُّ `delivered` مرّةً. الحكمُ `ok` في الطورَين. |
| حكمُ CI (PR #275) | `36139683721` · `36139691211` على `15c1f15`: السبعُ خضراءُ. في وظيفةِ PostgreSQL (الخطوةُ المُسمّاةُ ثمَّ مجلَّدُ التكاملِ): `/gate` تحريرُ القفلِ 1ms · تسلُّمٌ 1169/1172ms · رسالةٌ واحدةٌ من الثاني؛ `/send` تحريرٌ 0/1ms · تسلُّمٌ 1168/1167ms · رسالتانِ (الميّتُ · الثاني)؛ محاولتانِ و`delivered` وإعلانُ الميّتِ مرفوضٌ والحكمُ `ok` بصفرِ مخالفاتٍ في الجولاتِ الأربعِ. |
| ما لا يُدَّعى | لا قلبَ إلى `[x]` (`ح-4`)، ولا «مُثبَتٌ» (`ح-5`): البندُ في القسمِ الحادي عشرَ يُقصَدُ به **أثناءَ الحملِ وعلى بيئةٍ شبيهةٍ بالإنتاجِ** (`F9-01` · `F10-*`)؛ والمقيسُ مهمّةُ الصادرِ وحدَها. |

### Reservation `D-36` — **إخفاقُ تحميلِ جلسةِ الحوارِ يُقرأُ غياباً فيُقالُ للمستخدمِ «لم أفهم»** (opened 2026-09-25, before any file was edited · **merged 2026-09-25 في PR #274**)

اكتُشِفَ أثناءَ `F11-03` (`ADR 0193`) وحُجِزَ **قبلَ أيِّ تعديلٍ عليه**؛ يُنفَّذُ في فرعٍ مستقلٍّ من `main` بعدَ دمجِ `F11-03`.

| الحقلُ | القيمةُ |
|---|---|
| المقيسُ | `packages/application/bots/rider-dialog.ts` (`loadState`) و`driver-dialog.ts` (`loadState`): `if (!stored.ok) return INITIAL_STATE` — إخفاقُ المخزنِ وغيابُ الجلسةِ فرعٌ واحدٌ. في `tests/real-redis/redis-loss-commercial-state-real.test.ts` نقطتا الراكبِ أثناءَ القطعِ تلقّتا «🤔 لم أفهم هذه الرسالة» لا `common.error_try_again`. |
| الأثرُ | ردٌّ يُحمِّلُ المستخدمَ عطلَنا؛ وفي انقطاعٍ جزئيٍّ (قراءةٌ تسقطُ وكتابةٌ تمرُّ) قد تُكتَبُ `INITIAL_STATE` فوقَ جلسةٍ حقيقيّةٍ. |
| القيدُ | النقراتُ التي مصدرُ حقيقتِها القاعدةُ (`offer:` · `ride:` · `rate:` وأشباهُها) **تبقى تعملُ** بلا جلسةٍ — هيَ ما جعلَ `F11-03` أخضرَ؛ فلا يُسقَطُ كلُّ تحديثٍ لأنَّ الجلسةَ لم تُقرَأْ. |
| النطاقُ المحجوزُ | `packages/application/bots/rider-dialog.ts` · `driver-dialog.ts` · اختباراتُ الوحدةِ المقابلةُ · `ADR 0194` · دليلٌ · سجلُّ الديونِ · §25 · `SYSTEM_STATE` |
| ما لا يُمَسُّ | مخزنُ الجلساتِ ومحوِّلُ Redis · نصوصُ `ar.json` القائمةُ · حَكَمُ `F11-03` |
| توسيعُ النطاقِ (محجوزٌ بهذا الصفِّ قبلَ تعديلِه) | `packages/application/bots/session-read.ts` (مصدرٌ واحدٌ للقراءةِ في البوتَين) · `tests/integration/redis-sessions.test.ts`: حالةُ «لا يُفسَّر ردّ المستخدم…» كانت تُوكِّدُ «/help» تطبيقاً لـ`ADR 0011` §6، فعُدِّلَ توكيدُها بالإضافةِ وبقيَ ما تحميه (لا يُقبَلُ النصُّ اسماً · لا صفَّ) · زيادةٌ على `ADR 0011` · توكيدُ الردِّ الصادقِ في اختبارِ `F11-03` (كانَ مُسجَّلاً لا مُوكَّداً). |
| القرارُ | الإخفاقُ موسومٌ (`{state, unreadable}`)؛ المُدخَلُ المعتمدُ على الطورِ ⇒ عطلٌ صادقٌ بلا كتابةٍ؛ النصُّ الحرُّ يُنقَلُ إلى تفاوضٍ قائمٍ أوّلاً؛ ونبضةُ موقعِ السائقِ تبقى وثمنُها مكتوبٌ (`ADR 0194`). |
| المقيسُ محلّيّاً | ستُّ حالاتِ وحدةٍ تسقطُ على الشيفرةِ السابقةِ وتمرُّ بعدَها، وأربعٌ تحرسُ ما يبقى · `tests/unit` 5655/0 · `redis-sessions` 9/0 · `F11-03`: ردّا الراكبِ «حدث عطل تقني مؤقّت» والحكمُ `ok`. |
| حكمُ CI (PR #274) | `36137288886` على `69b0c86`: السبعُ خضراءُ. حالاتُ «متعذِّرةُ القراءةِ — D-36» العشرُ خضراءُ؛ و`redis-sessions` («لا يُفسَّر ردّ المستخدم…») خضراءُ في وظيفةِ PostgreSQL؛ وفي وظيفةِ Redis ردّا الراكبِ أثناءَ القطعِ «حدث عطل تقني مؤقّت. حاول بعد قليل.» وحَكَمُ `F11-03` `ok` بصفرِ مخالفاتٍ. |

### Reservation `F11-03` (الشقُّ المملوكُ للمستودَعِ) — **فقدانُ Redis: لا فسادَ في حالةٍ تجاريّةٍ — مقيسٌ لحوارِ التسجيلِ على مزدوجٍ وللمخزنِ الساخنِ وحدَه على خادمٍ حقيقيٍّ، ولم يُقطَعْ Redis كلُّه وسطَ دورةِ رحلةٍ قطُّ** (opened 2026-09-25, before any file was edited · **merged 2026-09-25 في PR #273**)

حُجِزَ **قبلَ أوّلِ تعديلِ ملفٍّ**، وفقَ قاعدةِ الحجزِ في `docs/ROADMAP-MASTER.md` §25. فرعُ `feat/f11-03-redis-loss-commercial-state` من `main` (`5583475`). لا فرعَ ولا PR ولا تنفيذَ سابقٌ يمسُّ `F11-03` (قُرِئَ: `git branch -r` · `gh pr list` · `grep F11-03 docs ROADMAP.md`).

| الحقلُ | القيمةُ |
|---|---|
| المقيسُ قبلَ التنفيذِ | انقطاعُ Redis مقيسٌ في موضعينِ لا ثالثَ لهما: `tests/integration/redis-sessions.test.ts` (حوارُ **التسجيلِ** على **مزدوجٍ** في الذاكرةِ: لا صفَّ ناقصاً) و`tests/real-redis/location-hot-state-outage-real.test.ts` (`F4-08`: **المخزنُ الساخنُ وحدَه** يُقطَعُ على خادمٍ حقيقيٍّ). ولا اختبارَ يقطعُ **كلَّ** ما يمسُّ Redis (الجلساتُ · ناقلُ `Streams` · مخزنُ البثِّ الحيِّ · الحالةُ الساخنةُ · دلوُ الصادرِ · حصّةُ التوجيهِ) **وسطَ دورةٍ تجاريّةٍ** — إنشاءُ طلبٍ ← قبولٌ متسابقٌ ← بدءٌ ← إقفالٌ ← إعادةُ تسليمٍ — ثمَّ يحكمُ على ثوابتِ الحالةِ التجاريّةِ في PostgreSQL. |
| ما يُبنى | `tests/real-redis/redis-loss-commercial-state-real.test.ts`: بوّابةٌ من `buildContainer` بـ`sessionStore: "redis"` على خادمِ Redis حقيقيٍّ، وعميلٌ يقطعُ **كلَّ أمرٍ** عندَ حدِّ العميلِ بعينِ صورةِ `createUpstashRedis` حينَ يسقطُ `fetch` (`{ok:false, kind:"network"}`). أطوارٌ: قطعٌ قبلَ إنشاءِ الطلبِ · قطعٌ بعدَه وسائقانِ يتسابقانِ على القبولِ وإعادةُ تسليمٍ · بدءٌ وإقفالٌ أثناءَ القطعِ · عودةٌ وإعادةُ تسليمٍ متقادمةٍ. وحَكَمٌ نقيٌّ `scripts/lib/commercial-state-invariants.ts` على لقطةٍ من القاعدةِ (طلبٌ واحدٌ للراكبِ · عرضٌ مقبولٌ واحدٌ على الأكثرِ وسائقُه هو المُسنَدُ · لا عرضَ معلَّقاً على طلبٍ غيرِ باحثٍ · سائقٌ برحلةٍ نشطةٍ واحدةٍ على الأكثرِ · حالةٌ نهائيّةٌ مشروعةٌ · لا حركةَ ماليّةَ لم يُطلَبْ مثلُها). |
| الإنفاذُ | يجري في وظيفةِ «تكامل على Redis حقيقي» (`REQUIRE_REAL_REDIS=1`: الغيابُ إخفاقٌ لا تخطٍّ)؛ وسالباتٌ مبذورةٌ (`ح-7`) تُثبِتُ أنَّ كلَّ قاعدةٍ في الحَكَمِ تسقطُ على لقطةٍ فاسدةٍ. |
| الوثائقُ | `ADR 0193` · دليلٌ في `docs/evidence/` · زيادةٌ على صفِّ `F11-03` في MASTER (`ح-1`: نصُّ البندِ لا يُمَسُّ) · §25 · `SYSTEM_STATE` |
| اكتشافٌ أثناءَ التنفيذِ (سُجِّلَ وحُجِزَ `D-36` قبلَ أيِّ تعديلٍ عليه) | ردُّ الراكبِ على نقطتَي الرحلةِ أثناءَ القطعِ «🤔 لم أفهم هذه الرسالة» مرّتين: `loadState` (`rider-dialog.ts` · `driver-dialog.ts`) تقرأُ **إخفاقَ** التحميلِ **غياباً** فتبدأُ من `INITIAL_STATE`. لا فسادَ تجاريّاً (الحَكَمُ أخضرُ)، لكنَّه ردٌّ غيرُ صادقٍ وخطرٌ كامنٌ في انقطاعٍ جزئيٍّ. لا يُعالَجُ في هذا الفرعِ ولا يُوكَّدُ نصُّه في اختبارِ `F11-03`. |
| توسيعُ النطاقِ (محجوزٌ بهذا الصفِّ) | `tests/unit/commercial-state-invariants.test.ts` (سالباتٌ) · مُدخلٌ في `scripts/lib/skip-registry.ts` (البوّابةُ `UPSTASH_REDIS_REST_URL` · يجري في خطوةِ «اختبارات الجلسات على Redis حقيقي» القائمةِ التي تُشغِّلُ المجلَّدَ كلَّه، فلا خطوةَ جديدةَ) · زيادةُ عدّادِ `tests/unit/skip-audit.test.ts` (143 · 1387) · صفُّ `D-36` في سجلِّ الديونِ. |
| حكمُ CI (PR #273) | `36134780670` · `36134778262` على `fd0fe4d`: السبعُ خضراءُ. في وظيفةِ «تكامل على Redis حقيقي»: 12 أمراً مقطوعاً (4 قبلَ الإنشاءِ · 8 من القبولِ إلى الإقفالِ) · مارٌّ 68 · طلبٌ واحدٌ `completed` · `accepted`/`cancelled` · الحكمُ `ok` بصفرِ مخالفاتٍ · كلُّ ويبهوكٍ (15) 200؛ والسالباتُ السبعَ عشرةَ خضراءُ. مطابقٌ للمحلّيِّ (مارٌّ 67 محلّيّاً). `F11-03` يبقى `[ ]`. |
| ما لا يُدَّعى | لا قلبَ إلى `[x]` (`ح-4`)، ولا «مُثبَتٌ» (`ح-5`): البندُ يُقصَدُ به **أثناءَ الحملِ وعلى بيئةٍ شبيهةٍ بالإنتاجِ** (`F9-01` · `F10-*`)، ولا سلطانَ للاختبارِ على خدمةٍ مُدارةٍ؛ والمقيسُ سلوكُ دورةٍ تجاريّةٍ واحدةٍ تحتَ فقدانٍ محقونٍ عندَ حدِّ العميلِ. |

### Reservation `F11-06` (الشقُّ المملوكُ للمستودَعِ) — **توقُّفُ مزوّدِ الخرائطِ: الرحلةُ لا تختفي، والمدّةُ تُخفى بصدقٍ — مكتوبٌ في الشيفرةِ ومُختبَرٌ بمزوّدٍ مُزيَّفٍ، ولم يُحقَنْ عطلٌ على السِلكِ قطُّ** (opened 2026-09-25, before any file was edited · **merged 2026-09-25 في PR #272**)
...
| حكمُ CI (PR #272) | `36130571776` · `36130579100` على `4f75248`: السبعُ خضراءُ. خطوةُ «(F11-06)» في وظيفةِ PostgreSQL: 6 pass · 0 fail — `stopped` 152ms/0 طلبٍ · `hanging` 3002ms/1 · `server_error` 152ms/2 · `malformed` 1ms/1 · قاطعٌ مفتوحٌ 0 طلبٍ ثمَّ `ROUTED` بعدَ 10254ms · `quota_exhausted` 0 طلبٍ؛ كلُّها 200 والرحلةُ مطابقةٌ و`PROVIDER_DOWN`. مطابقٌ للمحلّيِّ. `F11-06` يبقى `[ ]`. |

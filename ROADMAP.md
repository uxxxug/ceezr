...
### Reservation `SEC-19` — **`users.telegram_id` رابطُ ربطٍ لا مفتاحُ هويّةٍ** (opened 2026-09-22)

| الحقلُ | القيمةُ |
|---|---|
| البندُ | `SEC-19` (`docs/ROADMAP-MASTER.md:764`)، الحالةُ `[ ]` |
...
| **الطورُ الجاريُ 2026-09-22 — حرسُ العنوانِ الغائبِ في نقطةِ الخَنقِ (ساقُ «ب-٣»)** | `supabase/migrations/20260922050000_sec_19_claim_undeliverable_guard.sql` (expand) يُعيدُ كتابةَ `claim_notification_delivery()` بـ`create or replace` معَ إضافةِ فحصٍ واحدٍ بعدَ حلِّ العنوانِ: إذا كانَ `chat_id` غائبًا، يُعلَنُ الصفُّ `undeliverable` بـ`died_at` وسببٍ مُسمّىً، ويُعادُ `delivery: null`. والأسبابُ: `TELEGRAM_DELIVERY_UNAVAILABLE` للجوهريِّ (`negotiation_*`, `order_cancelled`) والسلامةِ (`safety_resolution_*`)، و`TELEGRAM_DELIVERY_NOT_REQUIRED` لغيرِ الجوهريِّ (`wider_circle_opened`, `no_driver_found`, `dispute_resolution`, `lost_item_report`). والاختبارُ `tests/integration/notification-outbox-claim-undeliverable-guard.test.ts` (٧ اختباراتٍ). ومدخلٌ في `scripts/lib/rollback-registry.ts`. **ولا يُقاسُ الفرعُ `offer`** — عنوانُهُ في TypeScript (الخطوةُ الرابعة). ولا تغييرَ في مسارِ التسليمِ الناجحِ — الحرسُ يقعُ بعدَ حلِّ العنوانِ وقبلَ الإرجاعِ. و`claim_token`/`claimed_at` يُنزَعانِ من الصفِّ `undeliverable`. |
| **عقبتانِ سابقتانِ كشفَهما التصنيفُ ولم يذكُرْهما الجردُ** | (١) **القناةُ البديلةُ معطوبةٌ**: `user_notifications` تُكتَبُ بـ`recipient_user_id` (أي `users.id`) بمُشغِّلٍ، **وتُقرَأُ بـ`get_user_notifications(p_telegram_id bigint)`** — فمن فُكَّ ربطُه تُكتَبُ إشعاراتُه ولا يقرؤها، **فالبديلُ يسقُطُ بسقوطِ الأصلِ لاشتراكِهما في المفتاحِ**. فلا يصحُّ تصنيفُ إشعارٍ «غيرَ جوهريٍّ» بحجّةِ البديلِ قبلَ إصلاحِ القارئِ. (٢) **حالاتُ الصندوقِ أربعٌ** `pending·sending·delivered·dead` وليسَ فيها «غيرُ قابلٍ للتسليمِ»، و`dead` تعني **فشلاً بعدَ جهدٍ** — فوضعُ «لا قناةَ أصلاً» فيها يخلِطُ ما أُمِرتُ بفصلِه ويُلوِّثُ كلَّ مقياسٍ. وكلتاهما هجرةٌ على `notification_outbox` **لا على `users`**. |
